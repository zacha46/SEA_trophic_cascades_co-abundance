...
caps = caps[caps$cell_id %in% meta$cell_id, ]
...
mat = matrix(NA, 
             nrow = length(unique(c$cell_id)), # number of rows are our sampling locations
             ncol = length(seq(from =1, to= max(c$seq))), # number of columns are our sampling occasions
             dimnames = list(as.character(unique(c$cell_id)), # row names, then column names
                             seq(from =1, to= max(c$seq))))

## Determine when each sampling unit was active-
for(j in 1:length(unique(c$cell_id))){
  
  a= c[c$cell_id == unique(c$cell_id)[j],] #subset for a specific sampling unit out of all possible options
...
for(j in 1:length(unique(c$cell_id))){ #repeat for each sampling unit
  ...
  su = unique(c$cell_id)[j] #specify the sampling unit
...
obs = distinct(dplyr::select(c, cell_id, seq, 
                             num_cams_active_at_date)) ## Come here and change to include more obs.covs if we get them!

## create empty obs dataframe
obs.mat = (matrix(NA, 
                  nrow = length(unique(obs$cell_id)), 
                  ncol = length(seq(from =1, to= max(obs$seq))),
                  dimnames = list(as.character(unique(obs$cell_id)), # row names, then column names
                                  seq(from = 1, to= max(obs$seq)))))

for(u in 1:length(unique(obs$cell_id))){ #repeat for each sampling unit
  
  # Select a single sampling unit (i.e. row)
  su = unique(obs$cell_id)[u]
  
  # Select data from a single sampling unit
  o = obs[obs$cell_id == su,]
  ...
  for(x in 1:max(o$seq)){ #repeat for each sequence 
    ...
    n = obs$num_cams_active_at_date[obs$cell_id == su & obs$seq == indx] ## COME HERE IF YOU HAVE MORE OBS COVS TO ADD! 
...
m = m[order(match(m$cell_id, rownames(dh.mat))),]


## and bind sampling unit name to obs and dh matricies
dh.mat = as.data.frame(cbind(m$cell_id, dh.mat))
names(dh.mat)[1] = "SU"

scaled.obs = as.data.frame(cbind(m$cell_id, scaled.obs))
