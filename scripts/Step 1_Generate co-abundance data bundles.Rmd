...
files = list.files("CT capture histories database/Asian ECL raw CT data/Step5_HPC_import_resampled_captures/")
...
## are leopards a preferred prey species for tigers? There is all sorts of literature saying they are subordinate, so maybe yes
traits$tiger_pref[traits$scientificNameStd == "Panthera_pardus"] = "Yes"
...
rm(i,l,sp_dom,sp_sub, sp_dom_dat, sp_sub_dat, obs, temp, z.dom, z.sub,
   bdata, all_else, data.dom, data.sub, covs, exp, non_det_SU, dom_tg, 
   non_det_land, dom_det, sub_det, lands, pres)
...
After some prelimiary testing, I realized that all co-abundance models should NOT run on the same amount of RAM... Most complete w/ 100 GB, but some need 250 or even 500 GB to complete. Rather than letting heavy models fail for a slow burn, I will reorganize the list into 100, 250, or 500 GB lists. This process will be acheived by inspecting the OE (output or error) files extracted from the HPC when each job runs. 
...
# ## assign each a GB level in a new DF
# GB = data.frame("number" = jobs, 
#                 "RAM" = c("100GB", "250GB", "500GB"))
...
for(i in 1:length(bdata_list)){
  ...
  for(f in 1:length(out)){
    ...
    }else{} # end finished mod conditional 
...
# ## Save data per GB requirements! 
# save = bdata_list[names(bdata_list) %in% jobs_final$Species_Pair[jobs_final$RAM == "100GB"]]
# saveRDS(save, here::here(paste("data/send_to_HPC/Bundled_data_for_Bayes_co-abundance_mods_100GB_", length(save), "_species_pairs_", date, ".RDS", sep = "")))
# 
# save = bdata_list[names(bdata_list) %in% jobs_final$Species_Pair[jobs_final$RAM == "250GB"]]
# saveRDS(save, here::here(paste("data/send_to_HPC/Bundled_data_for_Bayes_co-abundance_mods_250GB_", length(save), "_species_pairs_", date, ".RDS", sep = "")))
# 
# save = bdata_list[names(bdata_list) %in% jobs_final$Species_Pair[jobs_final$RAM == "500GB"]]
# saveRDS(save, here::here(paste("data/send_to_HPC/Bundled_data_for_Bayes_co-abundance_mods_500GB_", length(save), "_species_pairs_", date, ".RDS", sep = "")))
...
After running a meta-regression of hunting's effect on all species, we observed a counter-intuitive positive trend. Therefore, to make sure results are robust to the effect of hunting, I will save the 20 preferred models we have significant results for, and re-run them with log(hunting) and without the hunting var. Any sensitive results to hunting should then make itself clear. 

```{r generate testing bundles, echo=FALSE}

## import results of completed mods and results 
preform = read.csv("/Users/zachary_amir/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_trophic_cascades_co-abundance/results/summarized_results/model_performance_and_SIVs_20231215.csv")
# inspect
head(preform)

## grab the sp pairs that are supported and preferred
preform = preform[preform$support_liberal == "Supported" & preform$preference == "preferred",]

## verify which GBs we need here
table(jobs_final$RAM[jobs_final$Species_Pair %in% preform$Species_Pair]) # all options, lovely. 
# majority are 250, so work with that. 
preform = preform[preform$Species_Pair %in% jobs_final$Species_Pair[jobs_final$RAM == "250GB"],]

## who are we working with?
select(preform, Species_Pair, Significance, direction)
# significant top-down and bottom-up results. Good! 

## save em! 
# new_dat = bdata_list[names(bdata_list) %in% preform$Species_Pair]
# saveRDS(save, here::here(paste("data/send_to_HPC/Bundled_data_for_HUNTING_SENSITIVITY_co-abundance_mods_250GB_", length(new_dat), "_species_pairs_", date, ".RDS", sep = "")))
# rm(new_dat)
