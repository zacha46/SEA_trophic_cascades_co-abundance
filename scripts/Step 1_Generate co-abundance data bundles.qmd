...
length(keep) # 67 species after removing fluff. 
...
### Goal is to ensure EVERY species has appropriate species trait data
## and drop any species that does not 

### COME HERE!!! This is all copy/pasted from old code. 
## this needs to work for all 67 species!! 

## Thin datasets to relevant info
head(pantheria)
names(pantheria)
pan = select(pantheria, scientificNameStd, TrophicLevel, DietBreadth, AdultBodyMass_g)
head(pan)
rm(pantheria)

head(mammal_diet2)
names(mammal_diet2)
mam = select(mammal_diet2, scientificNameStd, TrophicLevel)
rm(mammal_diet2)

names(mam)[2] = "TrophicName"
mam = as.data.frame(mam) # remove silly tibble
## remove NA species names 
mam = mam[!is.na(mam$scientificNameStd),]

## check which species I looked at are missing from this DB
preform$sub_species = gsub("_", " ", preform$sub_species)

setdiff(preform$sub_species, pan$scientificNameStd) # onlt the genera-level critters and Tapir typo 
setdiff(preform$sub_species, mam$scientificNameStd) # the same 

## fix species name typo 
pan$scientificNameStd[pan$scientificNameStd == "Taperus indicus"] = "Tapirus indicus"
mam$scientificNameStd[mam$scientificNameStd == "Taperus indicus"] = "Tapirus indicus"

# and add misisng genera
pan[5511:(5511+5), "scientificNameStd"] = setdiff(preform$sub_species, pan$scientificNameStd)
mam[6716:(6716+5), "scientificNameStd"] = setdiff(preform$sub_species, mam$scientificNameStd)
# and associated info
pan[pan$scientificNameStd == "Capricornis sp", c("TrophicLevel","DietBreadth","AdultBodyMass_g")] = 
  pan[pan$scientificNameStd == "Capricornis sumatraensis", c("TrophicLevel","DietBreadth","AdultBodyMass_g")]
mam[mam$scientificNameStd == "Capricornis sp", "TrophicName"] = 
  mam[mam$scientificNameStd == "Capricornis sumatraensis", "TrophicName"]

pan[pan$scientificNameStd == "Herpestes sp", c("TrophicLevel","DietBreadth","AdultBodyMass_g")] = 
  pan[pan$scientificNameStd == "Herpestes urva", c("TrophicLevel","DietBreadth","AdultBodyMass_g")]
mam[mam$scientificNameStd == "Herpestes sp", "TrophicName"] = 
  mam[mam$scientificNameStd == "Herpestes urva", "TrophicName"]

pan[pan$scientificNameStd == "Muntiacus sp", c("TrophicLevel","DietBreadth","AdultBodyMass_g")] = 
  pan[pan$scientificNameStd == "Muntiacus muntjak", c("TrophicLevel","DietBreadth","AdultBodyMass_g")]
mam[mam$scientificNameStd =="Muntiacus sp", "TrophicName"] = 
  unique(mam[mam$scientificNameStd == "Muntiacus muntjak", "TrophicName"])

pan[pan$scientificNameStd == "Neofelis sp", c("TrophicLevel","DietBreadth","AdultBodyMass_g")] = 
  pan[pan$scientificNameStd == "Neofelis nebulosa", c("TrophicLevel","DietBreadth","AdultBodyMass_g")]
mam[mam$scientificNameStd == "Neofelis sp", "TrophicName"] = 
  mam[mam$scientificNameStd == "Neofelis nebulosa", "TrophicName"]

pan[pan$scientificNameStd == "Tragulus sp", c("TrophicLevel","DietBreadth","AdultBodyMass_g")] = 
  pan[pan$scientificNameStd == "Tragulus napu", c("TrophicLevel","DietBreadth","AdultBodyMass_g")]
mam[mam$scientificNameStd == "Tragulus sp", "TrophicName"] = 
  mam[mam$scientificNameStd == "Tragulus napu", "TrophicName"]

pan[pan$scientificNameStd == "Bos gaurus", c("TrophicLevel","DietBreadth","AdultBodyMass_g")] = 
  pan[pan$scientificNameStd == "Bos javanicus", c("TrophicLevel","DietBreadth","AdultBodyMass_g")]
mam[mam$scientificNameStd == "Bos gaurus", "TrophicName"] = 
  mam[mam$scientificNameStd == "Bos javanicus", "TrophicName"]

## double check
setdiff(preform$sub_species, pan$scientificNameStd) # full match! 
setdiff(preform$sub_species, mam$scientificNameStd) # full match! 


## thin pan and mam to match relevant species
pan = pan[pan$scientificNameStd %in% preform$sub_species,]
mam = mam[mam$scientificNameStd %in% preform$sub_species,]
mam = distinct(mam) # idk where repeats came in, but they did! 

## merge them
setdiff(pan$scientificNameStd, mam$scientificNameStd)
setdiff(mam$scientificNameStd, pan$scientificNameStd)
guilds = merge(pan, mam, by = "scientificNameStd")
head(guilds) # looks good! 
anyNA(guilds) # good! 
rm(pan, mam)

#### COME HERE! Examine trophic guilds closely and make sure to be explicit about adding an mesopredator guild. 

## add small and large to trophic names 
guilds$TrophicGuild = guilds$TrophicName

# inspect omnivores first
guilds$scientificNameStd[guilds$TrophicName == "Omnivore"] # thankfully not many, most are small
guilds$TrophicGuild[guilds$TrophicName == "Omnivore"] = "Small_Omnivore"
guilds$TrophicGuild[guilds$scientificNameStd %in% c("Helarctos malayanus",
                                                    "Sus scrofa", 
                                                    "Sus barbatus",
                                                    "Ursus thibetanus")] = "Large_Omnivore"
## "Ursus thibetanus" was classified as herbivore, but omnivore is more accurate. 

## herbivores
guilds$scientificNameStd[guilds$TrophicName == "Herbivore"] # almost all large
guilds$TrophicGuild[guilds$TrophicName == "Herbivore"] = "Large_Herbivore"
guilds$TrophicGuild[guilds$scientificNameStd %in% c("Muntiacus sp", 
                                                    "Macaca nemestrina",
                                                    "Macaca fascicularis",
                                                    "Tragulus sp")] = "Small_Herbivore"
## Carnivores
guilds$scientificNameStd[guilds$TrophicName == "Carnivore"] # pretty even split
guilds$TrophicGuild[guilds$scientificNameStd %in% c("Canis lupus", 
                                                    "Cuon alpinus",
                                                    "Neofelis sp",
                                                    "Panthera pardus",
                                                    "Panthera tigris")] = "Large_Carnivore"
guilds$TrophicGuild[guilds$scientificNameStd %in% c("Catopuma temminckii", 
                                                    "Hemigalus derbyanus",
                                                    "Herpestes sp",
                                                    "Manis javanica",
                                                    "Pardofelis marmorata",
                                                    "Prionailurus bengalensis",
                                                    "Prionodon linsang")] = "Small_Carnivore"
table(guilds$TrophicGuild) # looks good! 


###### Combine species trait data w/ model performance. 
head(preform)
head(guilds)
...
preform$dom_species = gsub("_", " ", preform$dom_species)
...
## add back the underscore to species names
preform$dom_species = gsub(" ", "_", preform$dom_species)
preform$sub_species = gsub(" ", "_", preform$sub_species)
