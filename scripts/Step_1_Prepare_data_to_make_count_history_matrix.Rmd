...
This code will prepare camera trap data to create count history matrices and the associated covariates. This is a necessary first step towards preparing the data to implement Zachary Amir's co-abundance models on the [High-Performance Computers (HPC)](https://rcc.uq.edu.au/systems/high-performance-computing/bunya).

This code works off the spatially re-sampled captures and covariates that are generated in the 4-step Southeast Asian camera trap data standardization pipeline. To learn more about this Southeast Asian camera trap data standardization pipeline, please contact [Zachary Amir](mailto:z.amir@uq.edu.au) or [Matthew Luskin](mailto:m.luskin@uq.edu.au) to request access to the relevant [GitHub repository](https://github.com/EcologicalCascadesLab/AsianCaptureHistories).
...
### Create a saving path to Dropbox of GitHub 
github = F # set to TRUE if saving to git 
if(github){
  save_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/data_GitHub/"
}else{
  save_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/data/step1_output/"
}
```
...
### This raw data is not available for public usage, but a derived and de-identified version will be hosted on GitHub 
...
## specify the working diretory where og data lives
...
# but add more here for Reviewer 2! 
land_sum = ddply(meta, .(Landscape), summarize,
                 num_su = (length(unique(cell_id))),
                 num_polygon = length(unique(Polygon5km)),
                 num_survey = length(unique(survey_id)),
                 duration = as.numeric(difftime(max(Sampling_end), min(Sampling_begin), units = "days")),
                 start_date = min(Sampling_begin),
                 end_date = max(Sampling_end),
                 data_sources = length(unique(source)),
                 # add bounding box for the spatial nerds out there... I C U! 
                 Xmin = min(Avg_Longitude),
                 Xmax = max(Avg_Longitude),
                 Ymin = min(Avg_Latitude),
                 Ymax = max(Avg_Latitude))

## create total_area_sampled by multiplying polygon count w/ 5
land_sum$total_area_sampled = land_sum$num_polygon*5
summary(land_sum$total_area_sampled) # good! Nothing over 1000 square km 
hist(land_sum$total_area_sampled) # classic poisson, who is the biggie?
land_sum$Landscape[land_sum$total_area_sampled > 700] # Eastern section of Thai forest reserve... could probably move that longitude a bit more East to make this smaller?

```

```{r Generate a table per landscape}
## The idea here is to provide more context for the Reviewer 2 who wanted to learn more about the data
## so this will be included as a supplementary table
head(land_sum) # and weve already got a start here, added all we can from the resampled data 

## extract relevant information from non-resampled meta data
land_sum_nonresamp = ddply(og_meta, .(Landscape), summarize,
                           num_dep = (length(unique(deployment_id))))
# make sure its safe to merge
setdiff(land_sum_nonresamp$Landscape, land_sum$Landscape)
setdiff(land_sum$Landscape, land_sum_nonresamp$Landscape) # safe to merge
land_sum = merge(land_sum, land_sum_nonresamp, by = "Landscape")
rm(land_sum_nonresamp)

## gather landscapes where each large carnivore was detected
tig_land = unique(meta$Landscape[meta$survey_id %in% caps$survey_id[caps$Species == "Panthera tigris"]])
leo_land = unique(meta$Landscape[meta$survey_id %in% caps$survey_id[caps$Species == "Panthera pardus"]])
dho_land = unique(meta$Landscape[meta$survey_id %in% caps$survey_id[caps$Species == "Cuon alpinus"]])
cl_land = unique(meta$Landscape[meta$survey_id %in% caps$survey_id[caps$Species %in% c("Neofelis diardi", "Neofelis nebulosa")]])
# and add these to land_sum
land_sum$tiger_det = "no"
land_sum$tiger_det[land_sum$Landscape %in% tig_land] = "yes"
land_sum$leopard_det = "no"
land_sum$leopard_det[land_sum$Landscape %in% leo_land] = "yes"
land_sum$dhole_det = "no"
land_sum$dhole_det[land_sum$Landscape %in% dho_land] = "yes"
land_sum$CL_det = "no"
land_sum$CL_det[land_sum$Landscape %in% cl_land] = "yes"
rm(tig_land, leo_land, dho_land, cl_land)

## finally, gather trap-nights from surv_sum, but need to add landscapes here!
add = distinct(select(surv_summary[surv_summary$survey_id %in% meta$survey_id,], survey_id, trap_nights))
add2 = distinct(select(meta, Landscape, survey_id))
add3 = merge(add, add2, by ="survey_id")
## now we need to summarize per landscape here
land_tn = ddply(add3, .(Landscape), summarize,
                sum_trap_nights = sum(as.numeric(trap_nights)))
# ensure a clean merge
setdiff(land_tn$Landscape, land_sum$Landscape)
setdiff(land_sum$Landscape, land_tn$Landscape) # were good! 
land_sum = merge(land_tn, land_sum, by = "Landscape")

## rename cols to be more clear
names(land_sum)
names(land_sum) = c("landscape", "total trap nights", "number of sampling units", "number of 5km polygons", "number of single-season surveys", "sampling duration", "start date", "end date", "number of data providers", "Xmin", "Xmax", "Ymin", "Ymax", "total area sampled km2", "number of deployments", "were tigers detected", "were leopards detected", "were dholes detected", "were clouded leopards detected")

# save this! 
# write.csv(land_sum, paste(save_path, "Landscape_summary_table_", 
#                           format(Sys.time(), format = "%Y%m%d"), ".csv", sep = ""), row.names = F)

## clean up
rm(add, add2, add3, land_tn, land_caps, cap_land)
...
__Tigers (*Panthera tigris*)__ were generated `r nrow(og_caps[og_caps$Species == "Panthera_tigris",])` independent detections from `r length(unique((og_caps$Landscape[og_caps$Species == "Panthera_tigris"])))` landscapes. They were deemed to prefer any other species that is not a large carnivore and above 17 kg. This includes a total of **`r length(traits$scientificNameStd[traits$tiger_pref == "Yes"])` species composed of `r paste(traits$scientificNameStd[traits$tiger_pref == "Yes"], collapse = ", ")`**. 

__Leopards (*Panthera pardus*)__ were generated `r nrow(og_caps[og_caps$Species == "Panthera_pardus",])` independent detections from `r length(unique((og_caps$Landscape[og_caps$Species == "Panthera_pardus"])))` landscapes. They were deemed to prefer any other species that is not a large carnivore and with a weight range of 10-40 kg. Also, all primates were included as preferred prey. This includes a total of **`r length(traits$scientificNameStd[traits$leopard_pref == "Yes"])` species composed of `r paste(traits$scientificNameStd[traits$leopard_pref == "Yes"], collapse = ", ")`**. 

__Dholes (*Cuon alpinus*)__ were generated `r nrow(og_caps[og_caps$Species == "Cuon_alpinus",])` independent detections from `r length(unique((og_caps$Landscape[og_caps$Species == "Cuon_alpinus"])))` landscapes. They were deemed to prefer any other species that is not a large carnivore and with a weight range of 40-190 kg. Also, all bear species were removed as preferred prey and muntjac deer were included. This includes a total of **`r length(traits$scientificNameStd[traits$dhole_pref == "Yes"])` species composed of `r paste(traits$scientificNameStd[traits$dhole_pref == "Yes"], collapse = ", ")`**. 
...
### Re-imagining group sizes here --> limit group size bases on mean AND max counts 
cap_counts = data.frame(matrix(nrow = 0, ncol = 2))
names(cap_counts) = c("Species", "capped_total_count")
for(sp in keep){
  
  # Grab all relevant counts for the current species
  sp_counts <- caps[["total_indiv_records"]][caps$Species == sp]
  ...
  res = cap_counts[0,]
  res[1, "Species"] = sp
  res[1, "capped_total_count"] = cap_val
  # combine w/ the total 
  cap_counts = distinct(rbind(res, cap_counts))
  
} # end per sp 

```

I have also determined which species need to be analyzed as groups instead of inviduals because of very large group sizes cause N-mixture models to crash and create unrealistic estimates. 

* Species with **mean counts greater than 10 or maximum counts greater than 20** were limited to 7 and this includes: `r paste(cap_counts$Species[cap_counts$capped_total_count == 7], collapse = ", ")`. 
* Species with **mean counts greater than 5 or maximum counts greater than 10** were limited to 5 and this includes: `r paste(cap_counts$Species[cap_counts$capped_total_count == 5], collapse = ", ")`. 
* Species with **mean counts greater than 3 or maximum counts greater than 5** were limited to 3 and this includes: `r paste(cap_counts$Species[cap_counts$capped_total_count == 3], collapse = ", ")`. 
* All other species did not have their counts limited, and this includes: `r paste(cap_counts$Species[cap_counts$capped_total_count == Inf], collapse = ", ")`. 
...
saveRDS(keep, paste(save_path, "species_vector_", length(keep), "_species_", scale, "_scale_", date, ".RDS", sep = ""))

saveRDS(group_sp, paste(save_path, "group_living_", length(group_sp), "_species_", scale, "_scale_",  date, ".RDS", sep = ""))

write.csv(caps, paste(save_path, "clean_captures_to_make_UMFs_", scale, "_scale_",  date,".csv", sep = ""), row.names = F)

write.csv(meta, paste(save_path, "clean_metadata_to_make_UMFs_", scale, "_scale_",  date,".csv", sep = ""), row.names = F)

## Also save trait data so I dont have to remake it later
write.csv(traits, paste(save_path, "clean_", nrow(traits), "_species_trait_data_", date, ".csv", sep = ""), row.names = F)
