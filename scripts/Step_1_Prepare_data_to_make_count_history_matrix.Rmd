...
land_sum = ddply(meta, .(Landscape), summarize,
                 ...
                 duration = as.numeric(difftime(max(Sampling_end), 
                                                min(Sampling_begin), units = "days")),
                 ...
                 # add mean for the three covariates
                 mean_flii = mean(Avg_FLLI),
                 mean_hfp  = mean(Avg_human_footprint),
                 mean_elev = mean(Avg_altitude)
                 )
...
## Summarize information per landscape from re-sampled data 
land_sum = ddply(meta, .(Landscape), summarize,
                 # deployment survey information 
                 num_su       = (length(unique(cell_id))),
                 num_polygon  = length(unique(Polygon5km)),
                 num_survey   = length(unique(survey_id)),
                 duration     = as.numeric(difftime(max(Sampling_end), 
                 ...
                 Ymin = min(Avg_Latitude),
                 Ymax = max(Avg_Latitude),
                 # add means for the three covariates
                 mean_flii = round(mean(Avg_FLLI), 3), 
                 mean_hfp  = round(mean(Avg_human_footprint), 3), 
                 mean_elev = round(mean(Avg_altitude), 3)
                 )
...
### incorporate number of non-resampled deployments from OG metadata
...
## clean up
rm(add, add2, add3, land_tn)

## rename cols to be more clear
names(land_sum)
names(land_sum) = c("landscape", "total trap nights", "number of sampling units", "number of 5km polygons", "number of single-season surveys", "sampling duration", "start date", "end date", "number of data providers", "Xmin", "Xmax", "Ymin", "Ymax", "mean forest integrity", "mean human footprint", "mean elevation", "total area sampled km2", "number of deployments", "were tigers detected", "were leopards detected", "were dholes detected", "were clouded leopards detected")
# save this! 
save_path = paste("~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/data/step1_output/Landscape_summary_table_",
                  format(Sys.Date(), "%Y%m%d"), ".csv", sep = "")
# write.csv(land_sum, save_path, row.names = F)
```
