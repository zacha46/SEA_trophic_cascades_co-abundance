...
if(github){
  save_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/data_CoA_bundles/"
  umf_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/data_UMFs/"
  import_dat_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/data_GitHub/"
  oe_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/data_GitHub/LONG_OE_files_Oct_2023/"
  cf_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/data_CoA_bundles/counterfactual_testing/"
}else{
  save_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/data/step2_output_CoA_bundles/"
  umf_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/data/UMFs/"
  import_dat_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/data/step1_output/"
  oe_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/results/LONG_OE_files_Oct_2023/"
  cf_path = "~//Dropbox/Zach PhD/Ch3 Trophic release project/data/step2_output_CoA_bundles/counterfactual_testing/"
}
...
for(i in 1:length(umfs)){
  ...
  mat_counts = as.numeric(as.vector(y))
  mat_counts = mat_counts[!is.na(mat_counts) & mat_counts > 0] # remove 0 and NAs 
  # max and mean 
  max_count = max(mat_counts)
  mean_count = round(mean(mat_counts),2)
  
  sp = names(umfs)[i]
  
  # #can print a message for us 
  # print(paste("The species", sp, "largest value in the matrix is", max_count, 
  #             "and the mean value in the matrix is", mean_count))
  r = res[0,]
  r[1, "species"] = sp
  r[1, "max_count"] = max_count
  res = dplyr::distinct(rbind(r,res))
  
}
rm(r, i, y, umf, mat_counts, max_count, mean_count, sp)

## inspect
summary(res$max_count) # median = 11, mean = 35. Max is crazy!! 
hist(res$max_count, breaks = 100) # big drop after 50 
hist(res$max_count[res$max_count<50], breaks = 100) # not much after 20

# which species
sort(res$species[res$max_count > 10]) # 24 species! no predators except dhole (makes sense)
sort(res$species[res$max_count > 20]) # 18 species, lots of group living species, no predators except dhole (makes sense)
sort(res$species[res$max_count > 25]) # 13 species
sort(res$species[res$max_count > 30]) # 11 species

## set the maximum to 7
cap_max = 7 #revert to 2 for testing if 7 is shit! 
cap_sp = sort(res$species[res$max_count > cap_max])

## loop thru each UMF 
for(i in 1:length(umfs)){
  # grab a umf and the dh matrix 
  u = umfs[[i]]
  y = u$y
  # Convert all columns except SU to numeric
  y[ , -which(names(y) == "SU")] <- lapply(y[ , -which(names(y) == "SU")], as.numeric)
  # Apply the transformation only to numeric columns
  y[ , sapply(y, is.numeric)] <- lapply(y[ , sapply(y, is.numeric)], function(col) {
    col[col > cap_max] <- cap_max
    return(col)
  })
  # resave
  umfs[[i]]$y = y
...
Finally, to limit over-dispersion in count history matrices, we have set the **maximum observed count in a 5-day sampling occasion window to `r cap_max`**, and this has been updated for the following species: `r paste(cap_sp, collapse = ", ")`.
...
jobs_final$preference[jobs_final$sub_sp == "Neofelis_genus" &
                        ...
                        traits$scientificNameStd[traits$CL_pref == "Yes"]] = "preferred"
## how many do we have?
table(jobs_final$preference) # 64 preferred, 194 not. 64 + 194 = 258, all accounted for! 
...
# saveRDS(save, paste(wd, "Bundled_data_for_Bayes_co-abundance_mods_preferred_", length(save), "_species_pairs_", date, ".RDS", sep = ""))
...
Instead, the new goal here is to examine **counterfactual realities** to understand if prey abundance increases would not have happened if not for large carnivore extirpation (top-down test), __AND__ if predator abundance increases would not have happened if not for prey abundance increases (bottom-up test). Additionally, we can run more variations of the same co-abundance model by sub-setting the cooccuring species data into locations that match in their measured covariates to help control for unmeasured variables. 

This approach builds towards the top of the **'ladder of causation'** proposed by [Judea Pearl's Book of Why](https://en.wikipedia.org/wiki/The_Book_of_Why). 

- The **first rung**, simple associations from data, can be represented simply with a non-directional association between two species (*e.g.,* correlation between RAI of two species). 
- The **second rung**, conceptual causal diagrams, can be used to visualize the researchers hypotheses, assumptions, and biases where one variable directionally affects another variable, such as predators suppress prey or prey bolster predators. 
- The **third rung**, testing counter-factual realities, forces the researcher to imagine an alternative reality where the assumed causal forces are reversed or eliminated, such as testing if prey abundance increases in a world with no large carnivore extirpation. 
...
comm_sp = as.vector("") # no community species during testing. 
...
## Adding Markdown text here for safe keeping, tho it wont be implemented. 
...
```{r Save bundled data for counterfactual test 6, eval=FALSE}

### MOVING the last line of the previous paragraph hashed out here for a clean knit
