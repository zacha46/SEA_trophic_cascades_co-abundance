...
# options are "Jan_2024", "Sep_2024" "Oct2024", "Dec2024", and "June_2025"
time = "June_2025"
...
sort(table(check$sub_species)) # same story -> sampling bias 
table(check$direction) # almost an even fit 
...
sort(table(check_td_dom$dom_species)) # all predators represented, but clouded leopards the most (but also with the most mods!)
# rm(check_td_dom)

## top-down subordinate 
check_td_sub = check[check$direction == "top-down" & check$BPV.sub < 0.15 | check$BPV.sub > 0.85, ]
sort(table(check_td_sub$sub_species)) # pigs, then sambar, then macaca 
...
sort(table(check_bu_sub$sub_species)) # mostly equal, tho tigers are missing (good!)
...
na.omit(check_bu$Species_Pair[duplicated(check_bu$Species_Pair)]) # all dominant species are classic group-size issue critters! 
...
```{r Import counter-factual test coefficents, eval=FALSE}
...
```{r Import counter-factual test PPC values, eval=FALSE}
# First, list all result files
# files = list.files(paste(wd, "results/", sep = ""), recursive = T)
files = list.files(wd, recursive = T) # update for github
# We dont want OE files (that informed GB requirements)
...
```{r Import counter-factual data, eval=FALSE}
...
```{r Determine support for counter-factual tests, eval=FALSE}
# 
...
```{r Determine agreement across counter-factual tests, eval=FALSE}
...
```{r text for counter factual html doc, eval=FALSE}
...
Based on the same criteria above for the original tests, the preferred models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "preferred"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "preferred"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "preferred"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "preferred"])` supported preferred models**.
Based on the same criteria above for the original tests, the community models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "community"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "community"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "community"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "community"])` supported community models**.
...
```{r save counterfactual results, eval=FALSE}
