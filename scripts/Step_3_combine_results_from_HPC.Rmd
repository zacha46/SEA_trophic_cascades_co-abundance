...
## Assign 3 levels of non-support --> wrong_direction, insignificant_siv, poor_fit
preform$support[preform$BPV_valid == "No" |
                  preform$OD_valid == "No" |
                  preform$parameter_valid == "No"] = "poor_fit" # lowest level of support --> bad mod.
preform$support[preform$BPV_valid == "Yes" & 
                  preform$OD_valid == "Yes" &
                  preform$parameter_valid == "Yes" &
                  preform$Significance == "Non-Significant"] = "insignificant_siv" # mid-low support --> good mod, but not important
preform$support[preform$BPV_valid == "Yes" & 
                  preform$OD_valid == "Yes" &
                  preform$parameter_valid == "Yes" &
                  preform$Significance == "Significant" &
                  preform$direction == "top-down" &
                  preform$Interaction_Estimate > 0] = "wrong_direction" # almost supportive --> good model, significant result, but not in correct direction for hypothesis.  
preform$support[preform$BPV_valid == "Yes" & 
                  preform$OD_valid == "Yes" &
                  preform$parameter_valid == "Yes" &
                  preform$Significance == "Significant" &
                  preform$direction == "bottom-up" &
                  preform$Interaction_Estimate < 0] = "wrong_direction" # Same as above, but applied for bottom-up direction. 
...
In combining these different diagnostic tools, we can determine different levels of support for our results. Models that had unsuitable parameter convergence or Bayes p-values and remaining over-dispersion are likely due to insufficient data or excessive residual noise from weakly interacting species (Fig. S2, poor_fit). Models that had insignificant SIVs are likely due to the inclusion of covariates in the model being more informative than species interactions (Fig. S2, insignificant_siv). Models with significant SIVs in the opposite direction as we hypothesized are likely due to shared responses to unexplained variation in covariates not included in the model, such as both predator and prey responding negatively to hunting in bottom-up models, or both species responding positively to fruit availability in top-down models (Fig. S2, wrong_direction). Finally, when a model meets all of the previous criteria, then we can determine the model as supported. The resulst from each level are provided below:

1) There were a total of `r length(preform$Species_Pair[preform$support == "poor_fit" & !is.na(preform$Interaction_Estimate)])` poor_fit models. This is composed of `r length(preform$Species_Pair[preform$support == "poor_fit" & !is.na(preform$Interaction_Estimate) & preform$preference == "preferred"])` preferred models and `r length(preform$Species_Pair[preform$support == "poor_fit" & !is.na(preform$Interaction_Estimate) & preform$preference == "community"])` community models. 

2) There were a total of `r length(preform$Species_Pair[preform$support == "insignificant_siv" & !is.na(preform$Interaction_Estimate)])` insignificant_siv models. This is composed of `r length(preform$Species_Pair[preform$support == "insignificant_siv" & !is.na(preform$Interaction_Estimate) & preform$preference == "preferred"])` preferred models and `r length(preform$Species_Pair[preform$support == "insignificant_siv" & !is.na(preform$Interaction_Estimate) & preform$preference == "community"])` community models. 

3) There were a total of `r length(preform$Species_Pair[preform$support == "wrong_direction" & !is.na(preform$Interaction_Estimate)])` wrong_direction models. This is composed of `r length(preform$Species_Pair[preform$support == "wrong_direction" & !is.na(preform$Interaction_Estimate) & preform$preference == "preferred"])` preferred models and `r length(preform$Species_Pair[preform$support == "wrong_direction" & !is.na(preform$Interaction_Estimate) & preform$preference == "community"])` community models. 
...
ppc_values_cf$support[ppc_values_cf$BPV_valid == "No" |
                        ppc_values_cf$OD_valid == "No" |
                        ppc_values_cf$parameter_valid == "No"] = "poor_fit" # lowest level of support --> bad mod.
ppc_values_cf$support[ppc_values_cf$BPV_valid == "Yes" & 
                        ppc_values_cf$OD_valid == "Yes" &
                        ppc_values_cf$parameter_valid == "Yes" &
                        ppc_values_cf$Significance == "Non-Significant"] = "insignificant_siv" # mid-low support --> good mod, but not important
ppc_values_cf$support[ppc_values_cf$BPV_valid == "Yes" & 
                        ppc_values_cf$OD_valid == "Yes" &
                        ppc_values_cf$parameter_valid == "Yes" &
                        ppc_values_cf$Significance == "Significant" &
                        ppc_values_cf$direction == "top-down" &
                        ppc_values_cf$Interaction_Estimate > 0] = "wrong_direction" # almost supportive --> good model, significant result, but not in correct direction for hypothesis.  
ppc_values_cf$support[ppc_values_cf$BPV_valid == "Yes" & 
                        ppc_values_cf$OD_valid == "Yes" &
                        ppc_values_cf$parameter_valid == "Yes" &
                        ppc_values_cf$Significance == "Significant" &
                        ppc_values_cf$direction == "bottom-up" &
                        ppc_values_cf$Interaction_Estimate < 0] = "wrong_direction" # Same as above, but applied for bottom-up direction. 
## assign which models support our hypothesis
ppc_values_cf$support[ppc_values_cf$BPV_valid == "Yes" & 
                        ppc_values_cf$OD_valid == "Yes" &
                        ppc_values_cf$parameter_valid == "Yes" &
...
Based on the same criteria above for the original tests, the preferred models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "preferred"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "preferred"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "preferred"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "preferred"])` supported preferred models**.
Based on the same criteria above for the original tests, the community models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "community"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "community"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "community"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "community"])` supported community models**.
...
Based on the same criteria above for the original tests, the preferred models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "preferred"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "preferred"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "preferred"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "preferred"])` supported preferred models**.
Based on the same criteria above for the original tests, the community models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "community"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "community"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "community"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "community"])` supported community models**.
...
Based on the same criteria above for the original tests, the preferred models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "preferred"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "preferred"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "preferred"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "preferred"])` supported preferred models**.
Based on the same criteria above for the original tests, the community models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "community"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "community"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "community"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "community"])` supported community models**.
...
Based on the same criteria above for the original tests, the preferred models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "preferred"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "preferred"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "preferred"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "preferred"])` supported preferred models**.
Based on the same criteria above for the original tests, the community models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "community"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "community"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "community"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "community"])` supported community models**.
...
Based on the same criteria above for the original tests, the preferred models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "preferred"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "preferred"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "preferred"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "preferred"])` supported preferred models**.
Based on the same criteria above for the original tests, the community models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "community"])` poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "insignificant_siv" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "community"])` insignificant_siv models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "community"])` wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "community"])` supported community models**.
