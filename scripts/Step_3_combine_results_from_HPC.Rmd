...
### Create import/saving paths to Dropbox or GitHub 
github = F # set to TRUE if saving to git 
if(github){
  import_legacy_dat_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/data_GitHub/"
  bundle_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/data_CoA_bundles/"
  hpc_results_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/data_CoA_bundles/results_final/"
  save_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/results_final/step3_output_combined_results/"
}else{
  import_legacy_dat_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/data/step1_output/"
  bundle_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/data/step2_output_CoA_bundles/"
  hpc_results_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/results/"
  save_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/data/step3_output_combined_results/"
}
...
wd = import_legacy_dat_path 

## Import clean cam trap data here for referencing
og_resamp_captures = read.csv(paste(wd,
                     "clean_captures_to_make_UMFs_5km_scale_20250306.csv", sep = ""))

og_resamp_meta = read.csv(paste(wd,
                          "clean_metadata_to_make_UMFs_5km_scale_20250306.csv", sep = ""))
...
# currently only interested in preferred
# files = files[grepl("preferred", files)]
# and must have GB
files = files[grepl("GB", files)]
...
for(i in 1:length(files)){
  ...
  # import
  b = readRDS(paste(wd, files[i], sep = ""))
  # and save
  res[[i]] = b
...
# options are "Jan_2024", "Sep_2024" "Oct2024", "Dec2024", "June_2025", and "July_2025"
time = "Sep_2024"
...
for(i in 1:length(files)){
  ...
  # import 
  d = read.csv(paste(wd, file, sep = ""))
  # d = read.csv(paste("results_final/", file, sep = "")) # update for github
...
### Inspect which species pairs are present/missing, 20250604, MIDDLE settings
setdiff(all_combos, coeff$Species_Pair) 
## 1 models missing --> "SUB-Panthera_pardus~DOM-Muntiacus_genus", position 13
# "one node produced an error: Error in node y.dom[716,3]"
...
## Fully completed by Feb 2025

## 1 missing from MIDDLE preferred testing
...
files = files[!grepl("OE", files)]
files = files[!grepl("counterfactual1|counterfactual2|counterfactual3|counterfactual4|counterfactual5|counterfactual6", files)]
...
### Inspect which species pairs are present/missing, 20250609, MIDDLE settings
setdiff(all_combos, ppc_values$Species_Pair) 
## same as missing above 
...
#load guild data
guilds = read.csv(paste(import_legacy_dat_path, "clean_44_species_trait_data_20240928.csv", sep = ""))
...
# preform$BPV_valid = "Yes" # for testing, lets ignore Bayes p-value for now. 
...
preform$parameter_valid[preform$Rhat >= 0.99 & preform$Rhat <= 1.1] = "Yes"
...
## Assign 3 levels of non-support --> 
# unsupported_wrong_direction (old version was unsupported_1) 
# unsupported_unclear_SIV (old version was unsupported_2) 
# unsupported_poor_fit (old version was unsupported_3)
preform$support[preform$BPV_valid == "No" |
                  preform$OD_valid == "No" |
                  preform$parameter_valid == "No"] = "unsupported_poor_fit" # lowest level of support --> bad mod.
preform$support[preform$BPV_valid == "Yes" & 
                  preform$OD_valid == "Yes" &
                  preform$parameter_valid == "Yes" &
                  preform$Significance == "Non-Significant"] = "unsupported_unclear_SIV" # mid-low support --> good mod, but not important
preform$support[preform$BPV_valid == "Yes" & 
                  preform$OD_valid == "Yes" &
                  preform$parameter_valid == "Yes" &
                  preform$Significance == "Significant" &
                  preform$direction == "top-down" &
                  preform$Interaction_Estimate > 0] = "unsupported_wrong_direction" # almost supportive --> good model, significant result, but not in correct direction for hypothesis.  
preform$support[preform$BPV_valid == "Yes" & 
                  preform$OD_valid == "Yes" &
                  preform$parameter_valid == "Yes" &
                  preform$Significance == "Significant" &
                  preform$direction == "bottom-up" &
                  preform$Interaction_Estimate < 0] = "unsupported_wrong_direction" # Same as above, but applied for bottom-up direction. 
...
## Assign 3 levels of non-support --> unsupported_wrong_direction, unsupported_unclear_SIV, unsupported_poor_fit
...
```{r Split out results on a per species pair basis}
## The idea here is that each species pair will only have one result -> top-down, bottom-up, or unsupported (w/ levels)
# and possibly try to average out the unsupported results??

# First, create consistent species pair ID in preform 
preform <- preform %>%
  mutate(pair_id = paste(pmin(dom_species, sub_species), # use pmin and pmax to alphabetically order 
                         pmax(dom_species, sub_species), # the species names so the direction doesn't matter.
                         sep = "_&_"))
# how many values?
length(unique(preform$pair_id)) # 143 --> b/c not EVERY model is repeated both direction (i.e., no small carnivore ~ large carnivore models)
head(preform$pair_id)

## Select the bare minimum of info needed here to ensure a clean reduction
preform_sp = select(preform, pair_id, direction, support, preference, Interaction_Estimate)

## first, verify there are no species pairs w/ more than 1 supported result
check <- preform_sp %>%
  group_by(pair_id) %>%
  summarise(
    num_supported = sum(support == "Supported"),
    .groups = "drop"
  )
summary(check$num_supported) # max is 1, good! 
# quickly check who is actually supported
check$pair_id[check$num_supported == 1] # 45 mods! 
rm(check)

## Pull out relevant results per pair
sp_pair_res = preform_sp[0, ] # fill the results into this empty DF 
for(i in 1:length(unique(preform_sp$pair_id))){
  # subset the data
  dat = preform_sp[preform_sp$pair_id == unique(preform_sp$pair_id)[i], ]
  # standardize unsupported levels to just unsupported
  dat$support[grepl("unsupported", dat$support)] = "unsupported"
  ## first check if there is support here
  if(any(dat$support == "Supported")){
    # if yes, take the supported result
    res = dat[dat$support == "Supported",]
    # and rbind w/ results
    sp_pair_res = rbind(sp_pair_res, res)
  }else{
    ## but if nothing is supported
    # take the average of the interaction estimate
    avg_int = mean(dat$Interaction_Estimate)
    # remove the col and make it distinct
    res = distinct(select(dat, -Interaction_Estimate))
    # then add in the average value 
    res$Interaction_Estimate = avg_int
    # and change direction to be neither
    res$direction = "unsupported"
    # finally rbind w/ results
    sp_pair_res = rbind(sp_pair_res, res)
  } # end support condition
} # end per pair_id
rm(res, avg_int, i, dat)

## Make sure we are distinct! 
sp_pair_res = distinct(sp_pair_res)

## make sure everyone is accounted for 
setdiff(sp_pair_res$pair_id, preform_sp$pair_id)
setdiff(preform_sp$pair_id, sp_pair_res$pair_id) # all good! all models present! 

# inspect
head(sp_pair_res)
length(unique(sp_pair_res$pair_id)) == nrow(sp_pair_res) # should be true, but its not! 
# which species pairs are preferred and in community?
sp_pair_res[sp_pair_res$direction == "top-down" & 
              sp_pair_res$preference == "preferred", ] # the 2 usual suspects, good! 
sp_pair_res[sp_pair_res$direction == "bottom-up" & 
              sp_pair_res$preference == "preferred", ] # the 12 usual suspects, good! 
## how many unsupported pairs?
nrow(sp_pair_res[sp_pair_res$support != "Supported" & sp_pair_res$preference == "community",]) # 80
nrow(sp_pair_res[sp_pair_res$support != "Supported" & sp_pair_res$preference == "preferred",]) # 18
# so 98 total unsupported pairs 

### How many total pairs?
length(unique(preform$pair_id)) # 143
length(sp_pair_res$pair_id) # 143, good! 

## How many top-down pairs, community + preffered
# Can only examine in sp_pair_res b/c preform counts pairs in both directions! 
# Can only talk about sp_pair_res as results, not in how many tests were implemented. 
length(sp_pair_res$pair_id[sp_pair_res$direction == "top-down" & 
                              sp_pair_res$preference == "community"]) # 14 supported top-down pairs 
length(sp_pair_res$pair_id[sp_pair_res$direction == "bottom-up" & 
                              sp_pair_res$preference == "community"]) # 17 supported bottom-up pairs 
# and now the preferred ones
length(sp_pair_res$pair_id[sp_pair_res$direction == "top-down" & 
                              sp_pair_res$preference == "preferred"]) # 2 supported top-down pairs 
length(sp_pair_res$pair_id[sp_pair_res$direction == "bottom-up" & 
                              sp_pair_res$preference == "preferred"]) # 12 supported top-down pairs 
...
When examining pairwise comparisons of species, each species-pair (regardless of the direction) can produce one result that is either top-down, bottom-up, or unsupported. There were a total of **`r length(sp_pair_res$pair_id[sp_pair_res$direction == "top-down"])` species-pairs supporting top-down regulation**, a total of **`r length(sp_pair_res$pair_id[sp_pair_res$direction == "bottom-up"])` species-pairs supporting bottom-up regulation**, and a total of **`r length(sp_pair_res$pair_id[sp_pair_res$direction == "unsupported"])` species-pairs supporting no associations**. These results can be further sub-divided into preferred & community results:

* For the **preferred species-pairs**, there were `r length(sp_pair_res$pair_id[sp_pair_res$direction == "top-down" & sp_pair_res$preference == "preferred"])` species-pairs supporting top-down regulation, a total of `r length(sp_pair_res$pair_id[sp_pair_res$direction == "bottom-up" & sp_pair_res$preference == "preferred"])` species-pairs supporting bottom-up regulation, and a total of `r length(sp_pair_res$pair_id[sp_pair_res$direction == "unsupported" & sp_pair_res$preference == "preferred"])` species-pairs supporting no associations.

* For the **community species-pairs**, there were `r length(sp_pair_res$pair_id[sp_pair_res$direction == "top-down" & sp_pair_res$preference == "community"])` species-pairs supporting top-down regulation, a total of `r length(sp_pair_res$pair_id[sp_pair_res$direction == "bottom-up" & sp_pair_res$preference == "community"])` species-pairs supporting bottom-up regulation, and a total of `r length(sp_pair_res$pair_id[sp_pair_res$direction == "unsupported" & sp_pair_res$preference == "community"])` species-pairs supporting no associations.

The settings used on the HPC to complete these models are composed of `r length(unique(preform$Species_Pair[preform$MCMC_setting == "LONG" & !is.na(preform$Interaction_Estimate)]))` models with long settings,  `r length(unique(preform$Species_Pair[preform$MCMC_setting == "MIDDLE" & !is.na(preform$Interaction_Estimate)]))` models with middle settings, and  `r length(unique(preform$Species_Pair[preform$MCMC_setting == "SHORT" & !is.na(preform$Interaction_Estimate)]))` models with short settings. 
...
1) There were a total of `r length(preform$Species_Pair[preform$support == "unsupported_poor_fit" & !is.na(preform$Interaction_Estimate)])` unsupported_poor_fit models. This is composed of `r length(preform$Species_Pair[preform$support == "unsupported_poor_fit" & !is.na(preform$Interaction_Estimate) & preform$preference == "preferred"])` preferred models and `r length(preform$Species_Pair[preform$support == "unsupported_poor_fit" & !is.na(preform$Interaction_Estimate) & preform$preference == "community"])` community models. 

2) There were a total of `r length(preform$Species_Pair[preform$support == "unsupported_unclear_SIV" & !is.na(preform$Interaction_Estimate)])` unsupported_unclear_SIV models. This is composed of `r length(preform$Species_Pair[preform$support == "unsupported_unclear_SIV" & !is.na(preform$Interaction_Estimate) & preform$preference == "preferred"])` preferred models and `r length(preform$Species_Pair[preform$support == "unsupported_unclear_SIV" & !is.na(preform$Interaction_Estimate) & preform$preference == "community"])` community models. 

3) There were a total of `r length(preform$Species_Pair[preform$support == "unsupported_wrong_direction" & !is.na(preform$Interaction_Estimate)])` unsupported_wrong_direction models. This is composed of `r length(preform$Species_Pair[preform$support == "unsupported_wrong_direction" & !is.na(preform$Interaction_Estimate) & preform$preference == "preferred"])` preferred models and `r length(preform$Species_Pair[preform$support == "unsupported_wrong_direction" & !is.na(preform$Interaction_Estimate) & preform$preference == "community"])` community models. 
...
* **For the supported top-down results, we had a total of `r length(preform$Species_Pair[preform$direction == "top-down" & preform$support == "Supported"])` models. This is composed of `r length(preform$Species_Pair[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "preferred"])` preferred models and `r length(preform$Species_Pair[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "community"])` community models.**
    + __Tigers__ showed `r length(preform$Species_Pair[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "preferred" & grepl("tigris", preform$dom_species)])` top-down relationships with their preferred prey and `r length(preform$Species_Pair[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "community" & grepl("tigris", preform$dom_species)])` top-down relationships with the overall community. In terms of distinct species-pairs, tigers exhibited top-down regulation in `r nrow(sp_pair_res[sp_pair_res$direction == "top-down" & grepl("tigris", sp_pair_res$pair_id) & sp_pair_res$preference == "preferred",])` preferred species-pairs and `r nrow(sp_pair_res[sp_pair_res$direction == "top-down" & grepl("tigris", sp_pair_res$pair_id) & sp_pair_res$preference == "community",])` in community pairs. 
    + __Leopards__ showed `r length(preform$Species_Pair[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "preferred" & grepl("pardus", preform$dom_species)])` top-down relationships with their preferred prey and `r length(preform$Species_Pair[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "community" & grepl("pardus", preform$dom_species)])` top-down relationships with the overall community. In terms of distinct species-pairs, leopards exhibited top-down regulation in `r nrow(sp_pair_res[sp_pair_res$direction == "top-down" & grepl("pardus", sp_pair_res$pair_id) & sp_pair_res$preference == "preferred",])` preferred species-pairs and `r nrow(sp_pair_res[sp_pair_res$direction == "top-down" & grepl("pardus", sp_pair_res$pair_id) & sp_pair_res$preference == "community",])` in community pairs.
    + __Dholes__ showed `r length(preform$Species_Pair[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "preferred" & grepl("Cuon", preform$dom_species)])` top-down relationships with their preferred prey and `r length(preform$Species_Pair[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "community" & grepl("Cuon", preform$dom_species)])` top-down relationships with the overall community. In terms of distinct species-pairs, dholes exhibited top-down regulation in `r nrow(sp_pair_res[sp_pair_res$direction == "top-down" & grepl("Cuon", sp_pair_res$pair_id) & sp_pair_res$preference == "preferred",])` preferred species-pairs and `r nrow(sp_pair_res[sp_pair_res$direction == "top-down" & grepl("Cuon", sp_pair_res$pair_id) & sp_pair_res$preference == "community",])` in community pairs.
    + __Clouded leopards__ showed `r length(preform$Species_Pair[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "preferred" & grepl("Neofelis", preform$dom_species)])` top-down relationships with their preferred prey and `r length(preform$Species_Pair[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "community" & grepl("Neofelis", preform$dom_species)])` top-down relationships with the overall community. In terms of distinct species-pairs, clouded leopards exhibited top-down regulation in `r nrow(sp_pair_res[sp_pair_res$direction == "top-down" & grepl("Neofelis", sp_pair_res$pair_id) & sp_pair_res$preference == "preferred",])` preferred species-pairs and `r nrow(sp_pair_res[sp_pair_res$direction == "top-down" & grepl("Neofelis", sp_pair_res$pair_id) & sp_pair_res$preference == "community",])` in community pairs.
    
## Supported bottom-up results 

* **For the supported bottom-up results, we had a total of `r length(preform$Species_Pair[preform$direction == "bottom-up" & preform$support == "Supported"])` models. This is composed of `r length(preform$Species_Pair[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "preferred"])` preferred models and `r length(preform$Species_Pair[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "community"])` community models.**
    + __Tigers__ showed `r length(preform$Species_Pair[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "preferred" & grepl("tigris", preform$sub_species)])` bottom-up relationships with their preferred prey and `r length(preform$Species_Pair[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "community" & grepl("tigris", preform$sub_species)])` bottom-up relationships with the overall community. In terms of distinct species-pairs, tigers exhibited bottom-up regulation in `r nrow(sp_pair_res[sp_pair_res$direction == "bottom-up" & grepl("tigris", sp_pair_res$pair_id) & sp_pair_res$preference == "preferred",])` preferred species-pairs and `r nrow(sp_pair_res[sp_pair_res$direction == "bottom-up" & grepl("tigris", sp_pair_res$pair_id) & sp_pair_res$preference == "community",])` in community pairs.
    + __Leopards__ showed `r length(preform$Species_Pair[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "preferred" & grepl("pardus", preform$sub_species)])` bottom-up relationships with their preferred prey and `r length(preform$Species_Pair[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "community" & grepl("pardus", preform$sub_species)])` bottom-up relationships with the overall community. In terms of distinct species-pairs, leopards exhibited bottom-up regulation in `r nrow(sp_pair_res[sp_pair_res$direction == "bottom-up" & grepl("pardus", sp_pair_res$pair_id) & sp_pair_res$preference == "preferred",])` preferred species-pairs and `r nrow(sp_pair_res[sp_pair_res$direction == "bottom-up" & grepl("pardus", sp_pair_res$pair_id) & sp_pair_res$preference == "community",])` in community pairs.
    + __Dholes__ showed `r length(preform$Species_Pair[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "preferred" & grepl("Cuon", preform$sub_species)])` bottom-up relationships with their preferred prey and `r length(preform$Species_Pair[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "community" & grepl("Cuon", preform$sub_species)])` bottom-up relationships with the overall community. In terms of distinct species-pairs, dholes exhibited bottom-up regulation in `r nrow(sp_pair_res[sp_pair_res$direction == "bottom-up" & grepl("Cuon", sp_pair_res$pair_id) & sp_pair_res$preference == "preferred",])` preferred species-pairs and `r nrow(sp_pair_res[sp_pair_res$direction == "bottom-up" & grepl("Cuon", sp_pair_res$pair_id) & sp_pair_res$preference == "community",])` in community pairs.
    + __Clouded leopards__ showed `r length(preform$Species_Pair[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "preferred" & grepl("Neofelis", preform$sub_species)])` bottom-up relationships with their preferred prey and `r length(preform$Species_Pair[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "community" & grepl("Neofelis", preform$sub_species)])` bottom-up relationships with the overall community. In terms of distinct species-pairs, clouded leopards exhibited bottom-up regulation in `r nrow(sp_pair_res[sp_pair_res$direction == "bottom-up" & grepl("Neofelis", sp_pair_res$pair_id) & sp_pair_res$preference == "preferred",])` preferred species-pairs and `r nrow(sp_pair_res[sp_pair_res$direction == "bottom-up" & grepl("Neofelis", sp_pair_res$pair_id) & sp_pair_res$preference == "community",])` in community pairs.

## Specific results per large carnivore

1) __Tigers preferred bottom-up relationships__ were with the following species: `r paste(sort(preform$dom_species[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "preferred" & grepl("tigris", preform$sub_species)]), collapse = ", & ")`, and the __preferred top-down relationships__ were with the following species: `r paste(sort(preform$sub_species[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "preferred" & grepl("tigris", preform$dom_species)]), collapse = ", & ")`. 
    + The __community bottom-up relationships__ were with the following species: `r paste(sort(preform$dom_species[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "community" & grepl("tigris", preform$sub_species)]), collapse = ", & ")`, and the __community top-down relationships__ were with the following species: `r paste(sort(preform$sub_species[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "community" & grepl("tigris", preform$dom_species)]), collapse = " & ")`. 

2) __Leopards preferred bottom-up relationships__ were with the following species: `r paste(sort(preform$dom_species[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "preferred" & grepl("pardus", preform$sub_species)]), collapse = ", & ")`, and the __preferred top-down relationships__ were with the following species: `r paste(sort(preform$sub_species[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "preferred" & grepl("pardus", preform$dom_species)]), collapse = ", & ")`. 
    + The __community bottom-up relationships__ were with the following species: `r paste(sort(preform$dom_species[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "community" & grepl("pardus", preform$sub_species)]), collapse = ", & ")`, and the __community top-down relationships__ were with the following species: `r paste(sort(preform$sub_species[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "community" & grepl("pardus", preform$dom_species)]), collapse = " & ")`. 

3) __Dholes preferred bottom-up relationships__ were with the following species: `r paste(sort(preform$dom_species[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "preferred" & grepl("Cuon", preform$sub_species)]), collapse = ", & ")`, and the __preferred top-down relationships__ were with the following species: `r paste(sort(preform$sub_species[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "preferred" & grepl("Cuon", preform$dom_species)]), collapse = ", & ")`. 
    + The __community bottom-up relationships__ were with the following species: `r paste(sort(preform$dom_species[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "community" & grepl("Cuon", preform$sub_species)]), collapse = ", & ")`, and the __community top-down relationships__ were with the following species: `r paste(sort(preform$sub_species[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "community" & grepl("Cuon", preform$dom_species)]), collapse = " & ")`. 

4) __Clouded leopards preferred bottom-up relationships__ were with the following species: `r paste(sort(preform$dom_species[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "preferred" & grepl("Neofelis", preform$sub_species)]), collapse = ", & ")`, and the __preferred top-down relationships__ were with the following species: `r paste(sort(preform$sub_species[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "preferred" & grepl("Neofelis", preform$dom_species)]), collapse = ", & ")`. 
    + The __community bottom-up relationships__ were with the following species: `r paste(sort(preform$dom_species[preform$direction == "bottom-up" & preform$support == "Supported" & preform$preference == "community" & grepl("Neofelis", preform$sub_species)]), collapse = ", & ")`, and the __community top-down relationships__ were with the following species: `r paste(sort(preform$sub_species[preform$direction == "top-down" & preform$support == "Supported" & preform$preference == "community" & grepl("Neofelis", preform$dom_species)]), collapse = " & ")`. 
...
```{r Inspect goodness of fit from new tests, eval=FALSE}

## which mods have invalid BPV?
table(preform$BPV_valid)
check = preform[preform$BPV_valid == "No",]
sort(table(check$dom_species)) # mostly predators 
sort(table(check$sub_species)) # same story -> sampling bias, tho mostly dhole! 
table(check$direction) # mostly top-down

## check which critter the poor fit lies with 
# top-down dominant species
check_td_dom = check[check$direction == "top-down" & check$BPV.dom < 0.15 | check$BPV.dom > 0.85, ]
sort(table(check_td_dom$dom_species)) # all predators represented, but dhole are the most by far. Group living species with issues?
# rm(check_td_dom)

## top-down subordinate 
check_td_sub = check[check$direction == "top-down" & check$BPV.sub < 0.15 | check$BPV.sub > 0.85, ]
sort(table(check_td_sub$sub_species)) #bears,  pigs, sambar, and 2 macaca sp all equally represented. 

## then combine to check for overlap 
check_td = rbind(check_td_sub, check_td_dom)
dim(check_td);dim(distinct(check_td)) # repeats are present
## which td mods had both species w/ poor fit?
na.omit(check_td$Species_Pair[duplicated(check_td$Species_Pair)]) # all subordinate species are classic group-size issue critters! 


# bottom-up subordinate species
check_bu_dom = check[check$direction == "bottom-up" & check$BPV.dom < 0.15 | check$BPV.dom > 0.85, ]
sort(table(check_bu_dom$dom_species)) # group size critters tend to be more common
# rm(check_td_dom)

## bottom-up subordinate 
check_bu_sub = check[check$direction == "bottom-up" & check$BPV.sub < 0.15 | check$BPV.sub > 0.85, ]
sort(table(check_bu_sub$sub_species)) #only dhole are present here... suspicuous! 

## then combine to check for overlap 
check_bu = rbind(check_bu_sub, check_bu_dom)
dim(check_bu);dim(distinct(check_bu)) # repeats are present
## which bu mods had both species w/ poor fit?
na.omit(check_bu$Species_Pair[duplicated(check_bu$Species_Pair)]) # Dhole is the only subordinate species here, then its all its possible prey... 

## inspect dhole specifically 
summary(preform$BPV.dom[grepl("DOM-Cuon_alpinus", preform$Species_Pair)]) # always too low 
summary(preform$BPV.sub[grepl("SUB-Cuon_alpinus", preform$Species_Pair)]) # also too low 
## inspect matrix 
mat_sub = as.vector(bdata$`SUB-Cuon_alpinus~DOM-Muntiacus_genus`$y.sub)
summary(mat_sub[!is.na(mat_sub) & mat_sub > 0])
# inspect the spread
hist(mat_sub[!is.na(mat_sub) & mat_sub > 0])
hist(log(mat_sub)[!is.na(mat_sub) & mat_sub > 0]) # log doesnt look much different, when we remove zeros and NA 
## repeat for dominant
mat_dom = as.vector(bdata$`SUB-Rusa_unicolor~DOM-Cuon_alpinus`$y.dom)
summary(mat_dom[!is.na(mat_dom) & mat_dom > 0]) # v similar (or same) as dom 
## how many observatons above 3?
mat_dom = mat_dom[!is.na(mat_dom) & mat_dom > 0] # remove zero and NA 
length(mat_dom[mat_dom>3]) # 63

### To contrast dholes, is there a species that always had a good fit?
sort(table(preform$sub_species[preform$support != "poor_fit" & 
                                 preform$direction == "bottom-up"])) # neofelis, followed by tigers for top-down mods 
sort(table(preform$dom_species[preform$support != "poor_fit" &
                                 preform$direction == "top-down"])) # neofelis for top-down mods ## inspect tiger and CL bpv values
summary(preform$BPV.dom[grepl("DOM-Neofelis_genus", preform$Species_Pair)])


## Lets also quickly inspect Rhat values for SIV
summary(preform$Rhat) # median is good. 
length(preform$Rhat[preform$Rhat > 1.2]) # 70 mods had bad convergence
table(preform$direction[preform$Rhat > 1.2]) # mostly top-down...
table(preform$dom_species[preform$Rhat > 1.2 & 
                            preform$direction == "top-down" &
                            preform$preference == "preferred"]) # mostly CL and leopard, others are equally represented

## Keep an eye out for underfitting!! Happens when bayes p-value is too small
summary(preform$BPV.dom[preform$direction == "top-down"]) # max = .39,  mean/median = 0.15
summary(preform$BPV.sub[preform$direction == "top-down"]) # max = .4, mean/media = 0.16
summary(preform$BPV.dom[preform$direction == "bottom-up"]) # max = .4,  mean/median = 0.2
summary(preform$BPV.sub[preform$direction == "bottom-up"]) # max = .4, mean/media = 0.2
# possibly due to tighgtly constrainted prior SIV parameter. 


# clean up 
rm(mat_dom, mat_sub, check_bu, check_bu_dom, 
   check_bu_sub, check_td, check_td_dom, check_td_sub)

```

```{r Determine which models are missing from results, eval=FALSE}
# isloate the missing species pairs 
missing_mods = preform$Species_Pair[is.na(preform$Interaction_Estimate)]
# create DF to store results 
res = data.frame(matrix(nrow = 0, ncol = 3))
names(res)[1:3] = c("Species_Pair", "data_file", "position")
# find all relevant data files
wd = paste("/Users/zachary_amir/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/data/")
files = list.files(paste(wd,"step2_output_CoA_bundles/", sep = ""))
# files = list.files(paste(getwd(),"/data_CoA_bundles/", sep = "")) # update for github
files = files[grepl("Bundled", files)]
files = files[!grepl("MISSING", files)] # avoid reading in repeated data 
# and must have GB
files = files[grepl("GB", files)]
# loop thru em 
for(i in 1:length(missing_mods)){
  # one sp pair
  sp = missing_mods[i]
  ...
  for(l in 1:length(files)){
    # import data
    dat = readRDS(paste(wd, "step2_output_CoA_bundles/", files[l], sep = ""))
    if(sp %in% names(dat)){
      # save in the results 
      res[i, "Species_Pair"] = sp
      res[i, "data_file"] = files[l]
      res[i, "position"] = which(names(dat) == sp)
    }else{
      # skip to next file 
      next
    } # end conditional for names 
  } # end per file
} # end per missing mod 
rm(files, i,l,sp)
...
```{r Import sensitivity test coefficents}

## establish where the results live 
wd = hpc_results_path
...
files_list = list("counterfactual1" = f1,
                  ...
                  "counterfactual4" = f4,
                  "counterfactual5" = f5#,
                  # "counterfactual6" = f6 # dont want this one anymore! 
                  )
...
for(i in 1:length(files_list)){
  ...
  if(length(fl)>0){
    ...
    # dat = read.csv(paste("results_final/", f, sep = "")) # update for github
...
```{r Import sensitivity test PPC values}
# First, list all result files
files = list.files(paste(wd, "results/", sep = ""), recursive = T)
# files = list.files(wd, recursive = T) # update for github
# We dont want OE files (that informed GB requirements)
files = files[!grepl("OE", files)]
...
```{r Import sensitivity data bundles}
## The idea is to track which models are completed vs not
cf_data_files = list.files(paste(wd, "data/step2_output_CoA_bundles/", sep = ""), recursive = T)
# cf_data_files = list.files("data_CoA_bundles/") # update for github 
## remove old files
cf_data_files = cf_data_files[! grepl("OLD", cf_data_files)]
## select for counterfactaul data
cf_data_files = cf_data_files[grepl("counterfactual", cf_data_files)]
...
for(i in 1:length(cf_data_files)){
  ...
  df = cf_mods[0,]
  # make a path
  path = paste(wd, "data/step2_output_CoA_bundles/", cf_data_files[i], sep = "")
  # path = paste("data_CoA_bundles/counterfactual_testing/", cf_data_files[i], sep = "") # update for github
  # import
  dat = readRDS(path)
  # grab the names
  sp = names(dat)
...
```{r Determine support for sensitivity tests}
# 
...
ppc_values_cf$parameter_valid[ppc_values_cf$Rhat >= 0.99 & ppc_values_cf$Rhat <= 1.1] = "Yes"
...
ppc_values_cf$support[ppc_values_cf$BPV_valid == "No" |
                        ppc_values_cf$OD_valid == "No" |
                        ppc_values_cf$parameter_valid == "No"] = "unsupported_poor_fit" # lowest level of support --> bad mod.
ppc_values_cf$support[ppc_values_cf$BPV_valid == "Yes" & 
                        ppc_values_cf$OD_valid == "Yes" &
                        ppc_values_cf$parameter_valid == "Yes" &
                        ppc_values_cf$Significance == "Non-Significant"] = "unsupported_unclear_SIV" # mid-low support --> good mod, but not important
ppc_values_cf$support[ppc_values_cf$BPV_valid == "Yes" & 
                        ppc_values_cf$OD_valid == "Yes" &
                        ppc_values_cf$parameter_valid == "Yes" &
                        ppc_values_cf$Significance == "Significant" &
                        ppc_values_cf$direction == "top-down" &
                        ppc_values_cf$Interaction_Estimate > 0] = "unsupported_wrong_direction" # almost supportive --> good model, significant result, but not in correct direction for hypothesis.  
ppc_values_cf$support[ppc_values_cf$BPV_valid == "Yes" & 
                        ppc_values_cf$OD_valid == "Yes" &
                        ppc_values_cf$parameter_valid == "Yes" &
                        ppc_values_cf$Significance == "Significant" &
                        ppc_values_cf$direction == "bottom-up" &
                        ppc_values_cf$Interaction_Estimate < 0] = "unsupported_wrong_direction" # Same as above, but applied for bottom-up direction. 
...
```{r Determine agreement across sensitivity tests}
...
```{r split out sensitivity tests on a per species pair basis}

# First, create consistent species pair ID in ppc_values_cf 
ppc_values_cf <- ppc_values_cf %>%
  mutate(pair_id = paste(pmin(dom_species, sub_species), # use pmin and pmax to alphabetically order 
                         pmax(dom_species, sub_species), # the species names so the direction doesn't matter.
                         sep = "_&_"))
# how many values?
length(unique(ppc_values_cf$pair_id)) # 143 --> b/c not EVERY model is repeated both direction (i.e., no small carnivore ~ large carnivore models)
head(ppc_values_cf$pair_id)

## Select the bare minimum of info needed here to ensure a clean reduction
preform_sp_cf = select(ppc_values_cf, pair_id, direction, support, preference, Interaction_Estimate, counter_factual_test, testing_agreement)

## Pull out relevant results per pair
sp_pair_res_cf = preform_sp_cf[0, ] # fill the results into this empty DF 
for(i in 1:length(unique(preform_sp_cf$pair_id))){
  # subset the data
  dat = preform_sp_cf[preform_sp_cf$pair_id == unique(preform_sp_cf$pair_id)[i], ]
  # standardize unsupported levels to just unsupported
  dat$support[grepl("unsupported", dat$support)] = "unsupported"
  ## first check if there is support here
  if(any(dat$support == "Supported")){
    ...
    res = dat[dat$support == "Supported",]
    # and rbind w/ results
    sp_pair_res_cf = rbind(sp_pair_res_cf, res)
  }else{
    ## but if nothing is supported
    # take the average of the interaction estimate
    avg_int = mean(dat$Interaction_Estimate)
...
## Make sure we are distinct! 
sp_pair_res_cf = distinct(sp_pair_res_cf)

## make sure everyone is accounted for 
setdiff(sp_pair_res_cf$pair_id, preform_sp_cf$pair_id)
setdiff(preform_sp_cf$pair_id, sp_pair_res_cf$pair_id) # all good! all models present! 

# inspect
head(sp_pair_res_cf)
length(unique(sp_pair_res_cf$pair_id)) == nrow(sp_pair_res_cf) # should be true, but its not! 
# which species pairs are preferred and in community?
sp_pair_res_cf[sp_pair_res_cf$direction == "top-down" & 
              sp_pair_res_cf$preference == "preferred" & 
                sp_pair_res_cf$testing_agreement == "agreement", ] # the 2 usual suspects, good! 
sp_pair_res_cf[sp_pair_res_cf$direction == "bottom-up" & 
              sp_pair_res_cf$preference == "preferred" & 
                sp_pair_res_cf$testing_agreement == "agreement", ] # the 9 sp pair, less than full test. Good! 
## how many unsupported pairs?
# nrow(sp_pair_res_cf[sp_pair_res_cf$support != "Supported" & sp_pair_res_cf$preference == "community",]) # 80
# nrow(sp_pair_res_cf[sp_pair_res_cf$support != "Supported" & sp_pair_res_cf$preference == "preferred",]) # 18
# so 98 total unsupported pairs 

### How many total pairs?
length(unique(preform$pair_id[preform$preference == "preferred"])) # 32
length(unique(sp_pair_res_cf$pair_id)) # 64 b/c just preferred species and the few extra mods. 
table(sp_pair_res_cf$preference)


## How many top-down pairs, community + preffered
# Can only examine in sp_pair_res_cf b/c preform counts pairs in both directions! 
# Can only talk about sp_pair_res_cf as results, not in how many tests were implemented. 
length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$direction == "top-down" & 
                                       sp_pair_res_cf$preference == "community" & 
                                       sp_pair_res_cf$testing_agreement == "agreement"])) # 4 supported top-down pairs 
length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$direction == "bottom-up" & 
                                       sp_pair_res_cf$preference == "community"& 
                                       sp_pair_res_cf$testing_agreement == "agreement"])) # 5 supported bottom-up pairs 
# and now the preferred ones
length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$direction == "top-down" & 
                                       sp_pair_res_cf$preference == "preferred"& 
                                       sp_pair_res_cf$testing_agreement == "agreement"])) # 2 supported top-down pairs 
length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$direction == "bottom-up" & 
                                       sp_pair_res_cf$preference == "preferred"& 
                                       sp_pair_res_cf$testing_agreement == "agreement"])) # 9 supported top-down pairs 

## quickly edit pair_id to look better in HTML output
preform$pair_id = gsub("_&_", "-&-", preform$pair_id) # underscores make text italic! 
sp_pair_res_cf$pair_id = gsub("_&_", "-&-", sp_pair_res_cf$pair_id)

```

We implemented a total of `r length(unique(ppc_values_cf$counter_factual_test)) - 1` sensitivity tests that will be compared with the original tests for a total of `r length(unique(ppc_values_cf$counter_factual_test))` tests. The key inference here will be determining which species pairs agree across all possible tests, noting that not every species pairing was able to be tested in every way. Currently, all tests have been run with the **MCMC setting `r paste(setting)`**. It is also important to note that while we ran counter factual tests for all `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$preference == "preferred"]))` preferred models, **we also ran counter factual tests for an additional `r length(unique(preform$Species_Pair[preform$preference == "community" & preform$support == "Supported"]))` community models that generated supported results in the original test**. 

* The first test, **`r paste(unique(ppc_values_cf$counter_factual_test)[1])`**, included a total of `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "preferred"]))` preferred models and `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "community"]))` community models. This equates to a total of `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & sp_pair_res_cf$preference == "preferred"]))` preferred species-pairs and `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & sp_pair_res_cf$preference == "community"]))` community species-pairs. 
    + Based on the same criteria above for the original tests, the _preferred results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "preferred"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "preferred"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "preferred"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "preferred"])` supported preferred models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "top-down"])` supported top-down preferred species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up preferred species-pairs.
    + Based on the same criteria above for the original tests, the _community results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "community"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "community"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "community"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & ppc_values_cf$preference == "community"])` supported community models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "top-down"])` supported top-down community species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[1] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up community species-pairs.

* The second test, **`r paste(unique(ppc_values_cf$counter_factual_test)[2])`**, included a total of `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "preferred"]))` preferred models and `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "community"]))` community models. This equates to a total of `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & sp_pair_res_cf$preference == "preferred"]))` preferred species-pairs and `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & sp_pair_res_cf$preference == "community"]))` community species-pairs.
    + Based on the same criteria above for the original tests, the _preferred results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "preferred"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "preferred"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "preferred"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "preferred"])` supported preferred models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "top-down"])` supported top-down preferred species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up preferred species-pairs.
    + Based on the same criteria above for the original tests, the _community results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "community"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "community"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "community"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & ppc_values_cf$preference == "community"])` supported community models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "top-down"])` supported top-down community species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[2] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up community species-pairs.

* The third test, **`r paste(unique(ppc_values_cf$counter_factual_test)[3])`**, included a total of `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & ppc_values_cf$preference == "preferred"]))` preferred models and `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & ppc_values_cf$preference == "community"]))` community models. This equates to a total of `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & sp_pair_res_cf$preference == "preferred"]))` preferred species-pairs and `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & sp_pair_res_cf$preference == "community"]))` community species-pairs.
    + Based on the same criteria above for the original tests, the _preferred results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & ppc_values_cf$preference == "preferred"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & ppc_values_cf$preference == "preferred"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & ppc_values_cf$preference == "preferred"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & ppc_values_cf$preference == "preferred"])` supported preferred models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "top-down"])` supported top-down preferred species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up preferred species-pairs.
    + Based on the same criteria above for the original tests, the _community results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & ppc_values_cf$preference == "community"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & ppc_values_cf$preference == "community"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & ppc_values_cf$preference == "community"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & ppc_values_cf$preference == "community"])` supported community models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "top-down"])` supported top-down community species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[3] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up community species-pairs.

* The fourth test, **`r paste(unique(ppc_values_cf$counter_factual_test)[4])`**, included a total of `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "preferred"]))` preferred models and `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "community"]))` community models. This equates to a total of `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & sp_pair_res_cf$preference == "preferred"]))` preferred species-pairs and `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & sp_pair_res_cf$preference == "community"]))` community species-pairs.
    + Based on the same criteria above for the original tests, the _preferred results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "preferred"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "preferred"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "preferred"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "preferred"])` supported preferred models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "top-down"])` supported top-down preferred species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up preferred species-pairs.
    + Based on the same criteria above for the original tests, the _community results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "community"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "community"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "community"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & ppc_values_cf$preference == "community"])` supported community models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "top-down"])` supported top-down community species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[4] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up community species-pairs.

* The fifth test, **`r paste(unique(ppc_values_cf$counter_factual_test)[5])`**, included a total of `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "preferred"]))` preferred models and `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "community"]))` community models. This equates to a total of `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & sp_pair_res_cf$preference == "preferred"]))` preferred species-pairs and `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & sp_pair_res_cf$preference == "community"]))` community species-pairs.
    + Based on the same criteria above for the original tests, the _preferred results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "preferred"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "preferred"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "preferred"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "preferred"])` supported preferred models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "top-down"])` supported top-down preferred species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up preferred species-pairs.
    + Based on the same criteria above for the original tests, the _community results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "community"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "community"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "community"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & ppc_values_cf$preference == "community"])` supported community models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "top-down"])` supported top-down community species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[5] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up community species-pairs.

* The sixth test, **`r paste(unique(ppc_values_cf$counter_factual_test)[6])`**, included a total of `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "preferred"]))` preferred models and `r length(unique(ppc_values_cf$Species_Pair[ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "community"]))` community models. This equates to a total of `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & sp_pair_res_cf$preference == "preferred"]))` preferred species-pairs and `r length(unique(sp_pair_res_cf$pair_id[sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & sp_pair_res_cf$preference == "community"]))` community species-pairs.
    + Based on the same criteria above for the original tests, the _preferred results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "preferred"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "preferred"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "preferred"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "preferred"])` supported preferred models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "top-down"])` supported top-down preferred species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & sp_pair_res_cf$preference == "preferred" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up preferred species-pairs.
    + Based on the same criteria above for the original tests, the _community results_ contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "community"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "community"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "community"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & ppc_values_cf$preference == "community"])` supported community models**. This equates to `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "top-down"])` supported top-down community species-pairs & `r length(sp_pair_res_cf$pair_id[sp_pair_res_cf$support == "Supported" & sp_pair_res_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[6] & sp_pair_res_cf$preference == "community" & sp_pair_res_cf$direction == "bottom-up"])` supported bottom-up community species-pairs.
...
* The **`r length(unique(preform$Species_Pair[preform$Species_Pair %in% agree_pref & preform$direction == "top-down" & preform$preference == "preferred"]))` preferred top-down models that agree across all sensitivity** tests are: `r paste(unique(preform$Species_Pair[preform$Species_Pair %in% agree_pref & preform$direction == "top-down" & preform$preference == "preferred"]), collapse = ", ")`. 
    + This equates to `r length(unique(preform$pair_id[preform$Species_Pair %in% agree_pref & preform$direction == "top-down" & preform$preference == "preferred"]))` species pairs composed of the following pairs: `r paste(unique(preform$pair_id[preform$Species_Pair %in% agree_pref & preform$direction == "top-down" & preform$preference == "preferred"]), collapse = ", ")`

* The **`r length(unique(preform$Species_Pair[preform$Species_Pair %in% agree_pref & preform$direction == "bottom-up" & preform$preference == "preferred"]))` preferred bottom-up models that agree across all sensitivity** tests are: `r paste(unique(preform$Species_Pair[preform$Species_Pair %in% agree_pref & preform$direction == "bottom-up" & preform$preference == "preferred"]), collapse = ", ")`.    
    + This equates to `r length(unique(preform$pair_id[preform$Species_Pair %in% agree_pref & preform$direction == "bottom-up" & preform$preference == "preferred"]))` species pairs composed of the following pairs: `r paste(unique(preform$pair_id[preform$Species_Pair %in% agree_pref & preform$direction == "bottom-up" & preform$preference == "preferred"]), collapse = ", ")`

* The **`r length(unique(preform$Species_Pair[preform$Species_Pair %in% agree_comm & preform$direction == "top-down" & preform$preference == "community"]))` community top-down models that agree across all sensitivity** tests are: `r paste(unique(preform$Species_Pair[preform$Species_Pair %in% agree_comm & preform$direction == "top-down" & preform$preference == "community"]), collapse = ", ")`.
    + This equates to `r length(unique(preform$pair_id[preform$Species_Pair %in% agree_comm & preform$direction == "top-down" & preform$preference == "community"]))` species pairs composed of the following pairs: `r paste(unique(preform$pair_id[preform$Species_Pair %in% agree_comm & preform$direction == "top-down" & preform$preference == "community"]), collapse = ", ")`

* The **`r length(unique(preform$Species_Pair[preform$Species_Pair %in% agree_comm & preform$direction == "bottom-up" & preform$preference == "community"]))` community bottom-up models that agree across all sensitivity** tests are: `r paste(unique(preform$Species_Pair[preform$Species_Pair %in% agree_comm & preform$direction == "bottom-up" & preform$preference == "community"]), collapse = ", ")`.
    + This equates to `r length(unique(preform$pair_id[preform$Species_Pair %in% agree_comm & preform$direction == "bottom-up" & preform$preference == "community"]))` species pairs composed of the following pairs: `r paste(unique(preform$pair_id[preform$Species_Pair %in% agree_comm & preform$direction == "bottom-up" & preform$preference == "community"]), collapse = ", ")`
...
### Also leaving all reporting code for the HTML document in here, hashed out
...
# Based on the same criteria above for the original tests, the preferred models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "preferred"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "preferred"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "preferred"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "preferred"])` supported preferred models**. Based on the same criteria above for the original tests, the community models contained `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_poor_fit" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "community"])` unsupported_poor_fit models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_unclear_SIV" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "community"])` unsupported_unclear_SIV models, `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "unsupported_wrong_direction" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "community"])` unsupported_wrong_direction models, **and `r length(ppc_values_cf$Species_Pair[ppc_values_cf$support == "Supported" & ppc_values_cf$counter_factual_test == unique(ppc_values_cf$counter_factual_test)[7] & ppc_values_cf$preference == "community"])` supported community models**.
...
# wd = paste(getwd(), "/results_final/step3_output_combined_results", sep = "")
...
### results per species pair 

# make a saving path 
path = paste(wd, "/compiled_co-abundance_species-pair_results_dataframe_", length(unique(sp_pair_res$pair_id[!is.na(sp_pair_res$Interaction_Estimate)])), "_completed_species-pairs_", date, ".csv", sep = "")
# and save it! 
write.csv(sp_pair_res, path)

### AND save the sensitivity test equivalent! 
path = paste(wd, "/compiled_co-abundance_species-pair_counterfactual_results_dataframe_", length(unique(sp_pair_res_cf$pair_id[!is.na(sp_pair_res_cf$Interaction_Estimate)])), "_completed_species-pairs_", date, ".csv", sep = "")
# and save it! 
write.csv(sp_pair_res_cf, path)
...
### Also, good idea to save the species we will need good prediciton graphs of later
