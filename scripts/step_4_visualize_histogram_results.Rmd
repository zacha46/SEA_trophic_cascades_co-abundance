...
## establish preferred plot style, either per "species" or "model"
plot_type = "species-pairs" # "models"

### Create import/saving paths to Dropbox or GitHub 
github = F # set to TRUE if saving to git 
if(github){
  import_results_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/results_final/step3_output_combined_results/"
  save_fig_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/figures/step4_output_figs/"
  save_dat_path = "~/GitHub Repositories/SEA_trophic_cascades_co-abundance/figures/step4_output_summarized_results_for_figs/"
}else{
  import_results_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/data/step3_output_combined_results/"
  save_fig_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/figures/step4_output_figs/"
  save_dat_path = "~/Dropbox/Zach PhD/Ch3 Trophic release project/SEA_TC_GitHub_data_storage/figures/step4_output_summarized_results_for_figs/"
}
...
wd = import_results_path

## list all relevant files
files = list.files(wd)

## dont want counter-factual results (yet)
files = files[! grepl("counter-factual|counterfactual", files)]

### Import performance results
p_file = files[grepl("preform", files)]
preform = read.csv(paste(wd, p_file, sep = ""))

## Also import results per species 
s_file = files[grepl("species-pair", files)]
preform_sp = read.csv(paste(wd, s_file, sep = ""))

## keep enviro clean
rm(p_file, s_file, files) 
...
```{r Prepare plotting data per model}

#
##
### Start by preparing plot_dat per model 
...
## and combine them
plot_dat_mod = rbind(td_plot_dat, bu_plot_dat)
rm(td_plot_dat, bu_plot_dat, a)

### verify all large-carnivore pairings are top-down only
table(plot_dat_mod$direction[plot_dat_mod$guild_pair == "SUB-Large_Carnivore~DOM-Large_Carnivore"]) # good! 

## set order manually b/c order is silly on graph
plot_dat_mod$rel_species = factor(plot_dat_mod$rel_species, levels = c("Neofelis_genus","Cuon_alpinus",
...
plot_dat_mod$support_simple = plot_dat_mod$direction
plot_dat_mod$support_simple_conserv = plot_dat_mod$direction

### edit support_simple to allow for unsupported mods
plot_dat_mod$support_simple[grepl("unsupported", plot_dat_mod$support)] = "unsupported"
table(plot_dat_mod$support_simple)

### and do the same for conservative values
plot_dat_mod$support_simple_conserv[grepl("unsupported", plot_dat_mod$support_conserv)] = "unsupported"
table(plot_dat_mod$support_simple_conserv)


## remove NA values from plot_dat_mod if present --> from non-completed mods
if(any(is.na(plot_dat_mod$Interaction_Estimate))){
  plot_dat_mod = plot_dat_mod[! is.na(plot_dat_mod$Interaction_Estimate), ]
}

## Check if there are extreme species interaction values (> |2|)
summary(plot_dat_mod$Interaction_Estimate)  # some large values overall

## Check pref models --> main text
summary(plot_dat_mod$Interaction_Estimate[plot_dat_mod$preference == "preferred"]) # narrow range here, all good! 


## set factors so unsupported is in the back 
plot_dat_mod$support_simple = factor(plot_dat_mod$support_simple, levels = c("top-down", "bottom-up", "unsupported"))
plot_dat_mod$support_simple_conserv = factor(plot_dat_mod$support_simple_conserv, levels = c("top-down", "bottom-up", "unsupported"))


## make the support column more informative
plot_dat_mod$support[plot_dat_mod$support == "Supported" & plot_dat_mod$direction == "top-down"] = "top-down"
plot_dat_mod$support_conserv[plot_dat_mod$support_conserv == "Supported" & plot_dat_mod$direction == "top-down"] = "top-down"
plot_dat_mod$support[plot_dat_mod$support == "Supported" & plot_dat_mod$direction == "bottom-up"] = "bottom-up"
plot_dat_mod$support_conserv[plot_dat_mod$support_conserv == "Supported" & plot_dat_mod$direction == "bottom-up"] = "bottom-up"

## make letters all lowercase
plot_dat_mod$support= tolower(plot_dat_mod$support)
plot_dat_mod$support_conserv = tolower(plot_dat_mod$support_conserv)
table(plot_dat_mod$support)

## and set factor level 
plot_dat_mod$support = factor(plot_dat_mod$support, levels = c("top-down", "bottom-up", "unsupported_wrong_direction", "unsupported_unclear_siv","unsupported_poor_fit"))
plot_dat_mod$support_conserv = factor(plot_dat_mod$support_conserv, levels = c("top-down", "bottom-up", "unsupported_wrong_direction", "unsupported_unclear_siv","unsupported_poor_fit"))

### Verify plot_dat_moda has the correct info 
table(plot_dat_mod$rel_species[plot_dat_mod$direction == "top-down"])
table(plot_dat_mod$rel_species[plot_dat_mod$direction == "bottom-up"]) # all critters accounted for 
## double check support
table(plot_dat_mod$support[plot_dat_mod$direction == "top-down"])
table(plot_dat_mod$support_conserv[plot_dat_mod$direction == "top-down"])
table(plot_dat_mod$support[plot_dat_mod$direction == "bottom-up"])
table(plot_dat_mod$support_conserv[plot_dat_mod$direction == "bottom-up"]) # All good! 

### set up colors for all values 
new_colors = c("unsupported_wrong_direction" = "gray87",
               "unsupported_unclear_siv" = "gray55",
               "unsupported_poor_fit" = "gray32",
               "unsupported" = "gray32",
...
```{r Prepare plotting data per species pair}
#
##
### Repeat the process, but this time for preform_sp 

# convert no_association to unsupported for color scheme to work
preform_sp$direction[preform_sp$direction == "no_association"] = "unsupported"

## first copy over preform to a new data frame 
plot_dat_sp = preform_sp
# and determine the relevant species
plot_dat_sp$rel_species[grepl("Panthera_tigris", plot_dat_sp$pair_id)] = "Panthera_tigris"
plot_dat_sp$rel_species[grepl("Panthera_pardus", plot_dat_sp$pair_id)] = "Panthera_pardus"
plot_dat_sp$rel_species[grepl("Cuon_alpinus", plot_dat_sp$pair_id)] = "Cuon_alpinus"
plot_dat_sp$rel_species[grepl("Neofelis_genus", plot_dat_sp$pair_id)] = "Neofelis_genus"
# now make a copy for all 
a = plot_dat_sp
a$rel_species = "All_large_carnivores"

## then combine into one 
plot_dat_sp = rbind(plot_dat_sp, a)
rm(a)

## set order manually b/c order is silly on graph
plot_dat_sp$rel_species = factor(plot_dat_sp$rel_species, levels = c("Neofelis_genus","Cuon_alpinus",
                                                               "Panthera_pardus","Panthera_tigris",
                                                                 "All_large_carnivores"))
table(plot_dat_sp$rel_species)

## create a new column for simple groupings: top-down, bottom-up and unsupported
plot_dat_sp$support_simple = plot_dat_sp$direction
plot_dat_sp$support_simple_conserv = plot_dat_sp$direction

### edit support_simple to allow for unsupported mods
plot_dat_sp$support_simple[grepl("unsupported", plot_dat_sp$support)] = "unsupported"
table(plot_dat_sp$support_simple)

### and do the same for conservative values
plot_dat_sp$support_simple_conserv[grepl("unsupported", plot_dat_sp$support_conserv)] = "unsupported"
table(plot_dat_sp$support_simple_conserv) # same as above, this is redundant! 

## remove NA values from plot_dat_sp if present --> from non-completed mods
if(any(is.na(plot_dat_sp$Interaction_Estimate))){
  plot_dat_sp = plot_dat_sp[! is.na(plot_dat_sp$Interaction_Estimate), ]
}

## Check if there are extreme species interaction values (> |2|)
summary(plot_dat_sp$Interaction_Estimate)  # some large values overall

## Check pref models --> main text
summary(plot_dat_sp$Interaction_Estimate[plot_dat_sp$preference == "preferred"]) # narrow range here, all good! 

## set factors so unsupported is in the back 
plot_dat_sp$support_simple = factor(plot_dat_sp$support_simple, levels = c("top-down", "bottom-up", "unsupported"))
plot_dat_sp$support_simple_conserv = factor(plot_dat_sp$support_simple_conserv, levels = c("top-down", "bottom-up", "unsupported"))


## make the support column more informative
plot_dat_sp$support[plot_dat_sp$support == "Supported" & plot_dat_sp$direction == "top-down"] = "top-down"
#plot_dat_sp$support_conserv[plot_dat_sp$support_conserv == "Supported" & plot_dat_sp$direction == "top-down"] = "top-down"
plot_dat_sp$support[plot_dat_sp$support == "Supported" & plot_dat_sp$direction == "bottom-up"] = "bottom-up"
#plot_dat_sp$support_conserv[plot_dat_sp$support_conserv == "Supported" & plot_dat_sp$direction == "bottom-up"] = "bottom-up"
## Just to make code work, copy supports
plot_dat_sp$support_conserv = plot_dat_sp$support

## make letters all lowercase
plot_dat_sp$support= tolower(plot_dat_sp$support)
plot_dat_sp$support_conserv = tolower(plot_dat_sp$support_conserv)

## and set factor level 
plot_dat_sp$support = factor(plot_dat_sp$support, levels = c("top-down", "bottom-up", "unsupported"))
plot_dat_sp$support_conserv = factor(plot_dat_sp$support_conserv, levels = c("top-down", "bottom-up", "unsupported"))

### Verify plot_dat_sp has the correct info 
table(plot_dat_sp$rel_species[plot_dat_sp$direction == "top-down"])
table(plot_dat_sp$rel_species[plot_dat_sp$direction == "bottom-up"]) # all critters accounted for 
## double check support
table(plot_dat_sp$support[plot_dat_sp$direction == "top-down"])
table(plot_dat_sp$support_conserv[plot_dat_sp$direction == "top-down"])
table(plot_dat_sp$support[plot_dat_sp$direction == "bottom-up"])
table(plot_dat_sp$support_conserv[plot_dat_sp$direction == "bottom-up"]) # All good!

```

```{r Explore histograms per species pair, eval=FALSE}
## This would be the main plot in figure 1, but reduced to sp-pairs
## and if this works/looks good, possibly replace the other figs w/ this style 

# make the plot 
ggplot(plot_dat_sp[plot_dat_sp$preference == "preferred",], aes(x = Interaction_Estimate, fill = direction))+
  ...
  scale_y_continuous(breaks = seq(0, max(table(preform_sp$direction)), by = 3)) +
  labs(x = "Species Interaction Value", y="Number of species pairs", fill = NULL, color = NULL)+
...
Importantly, histograms can be generated on a per model (i.e., each species pair has a top-down or bottom-up model), or per species pair (i.e., each pair is either top-down, bottom-up, or unsupported). **The histograms below have been calculated per `r plot_type`.**

```{r Determine if we want plots per model or per species}
## Make this a conditional statement so we dont have to change the code below! 
if(plot_type == "species-pairs"){
  plot_dat = plot_dat_sp
  hist_y = "Number of species pairs"
} 

if(plot_type == "models"){
  plot_dat = plot_dat_mod
  hist_y = "Number of pairwise co-abundance models"
}
```
...
ggplot(dist, aes(x = val, fill = label))+
  ...
  geom_vline(aes(xintercept = 0), linetype = "dashed", color = "firebrick4", alpha = .5)+
  scale_fill_manual(values = new_colors) +
  labs(x = "Species Interaction Value", y = hist_y, color = NULL, title = paste("CONCEPT"))+
  guides(fill = "none") + 
  theme(axis.text.x = element_text(size = 16),
        axis.text.y = element_text(size = 16),
        axis.text = element_text(color = "black"),
...
for(i in 1:length(order)){
  ...
  p = ggplot(d, aes(x = Interaction_Estimate, fill = support_simple))+
    ...
    labs(x = "Species Interaction Value", y = hist_y, fill = NULL, color = NULL, title = order[i])+
    theme(axis.text.x = element_text(size = 16),
          axis.text.y = element_text(size = 16),
          axis.text = element_text(color = "black"),
          text = element_text(family = "Helvetica"))
...
community_siv_all_large_carns_hist = 
  ...
  labs(x = "Species Interaction Value", y = hist_y, color = NULL, title = paste("all large carnivores and all prey"))+
  guides(fill = "none") + 
  theme(axis.text.x = element_text(size = 16),
        axis.text.y = element_text(size = 16),
        axis.text = element_text(color = "black"),
...
community_siv_all_large_carns_hist_cons = 
  ...
  labs(x = "Species Interaction Value", y = hist_y, color = NULL, title = paste("all large carnivores and all prey conservative support"))+
  guides(fill = "none") + 
  theme(axis.text.x = element_text(size = 16),
        axis.text.y = element_text(size = 16),
        axis.text = element_text(color = "black"),
...
## and combine them
pref_dat = plot_dat[plot_dat$preference == "preferred",] #rbind(td_dat_pref, bu_dat_pref)
...
# prob_pair_pref = unique(pref_dat$Species_Pair[pref_dat$Interaction_Estimate > 1.6]) # no longer present! Was probably removed w/ updated Rhats 

## custom-make all large carnivore graph for sizing
preferred_siv_all_large_carns_hist = 
  ggplot(pref_dat[pref_dat$rel_species == "All_large_carnivores",],
  ...
  labs(x = "Species Interaction Value", y = hist_y, color = NULL, title = paste("all large carnivores and preferred prey"))+
  guides(fill = "none") + 
  coord_cartesian()+
  theme(axis.text.x = element_text(size = 16),
        axis.text.y = element_text(size = 16),
...
perc_table = ddply(plot_dat, .(rel_species, support_simple), summarize,
                   num_lvl = length(Interaction_Estimate))
...
## and the response variable type 
perc_table$response_var = plot_type
...
## and the response variable type 
perc_table_cons$response_var = plot_type
...
## and the response variable type 
pref_perc_table$response_var = plot_type
...
## and the response variable type 
pref_perc_table_cons$response_var = plot_type
...
## Before we begin, these can only work via model, so we can see the different levels of un-support
if(plot_type == "species-pairs"){
  plot_dat = plot_dat_mod
}
...
Finally, the last set of histograms visualize the distribution of the unsupported results for preferred predator-prey models, split between top-down and bottom-up models. **When pooling data across all large carnivores for the top-down models**, there were `r nonsupport$num_lvl[nonsupport$rel_species == "All_large_carnivores" & nonsupport$PPC_setting == "normal" & nonsupport$direction == "top-down" & nonsupport$support_levels == "unsupported_poor_fit"]` unsupported_poor_fit models (`r paste(nonsupport$percent[nonsupport$rel_species == "All_large_carnivores" & nonsupport$PPC_setting == "normal" & nonsupport$direction == "top-down" & nonsupport$support_levels == "unsupported_poor_fit"], "%", sep = "")`), `r nonsupport$num_lvl[nonsupport$rel_species == "All_large_carnivores" & nonsupport$PPC_setting == "normal" & nonsupport$direction == "top-down" & nonsupport$support_levels == "unsupported_unclear_siv"]` unsupported_unclear_siv models (`r paste(nonsupport$percent[nonsupport$rel_species == "All_large_carnivores" & nonsupport$PPC_setting == "normal" & nonsupport$direction == "top-down" & nonsupport$support_levels == "unsupported_unclear_siv"], "%", sep = "")`), and `r nonsupport$num_lvl[nonsupport$rel_species == "All_large_carnivores" & nonsupport$PPC_setting == "normal" & nonsupport$direction == "top-down" & nonsupport$support_levels == "unsupported_wrong_direction"]` unsupported_poor_fit models (`r paste(nonsupport$percent[nonsupport$rel_species == "All_large_carnivores" & nonsupport$PPC_setting == "normal" & nonsupport$direction == "top-down" & nonsupport$support_levels == "unsupported_wrong_direction"], "%", sep = "")`).

* The top-down results for `r unique(nonsupport$rel_species)[4]` include `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[4] & nonsupport$support_levels == "unsupported_wrong_direction" & nonsupport$direction == "top-down"]` unsupported_wrong_direction models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[4] & nonsupport$support_levels == "unsupported_wrong_direction" & nonsupport$direction == "top-down"], "%", sep = "")`), `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[4] & nonsupport$support_levels == "unsupported_unclear_siv" & nonsupport$direction == "top-down"]` unsupported_unclear_siv models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[4] & nonsupport$support_levels == "unsupported_unclear_siv" & nonsupport$direction == "top-down"], "%", sep = "")`), and `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[4] & nonsupport$support_levels == "unsupported_poor_fit" & nonsupport$direction == "top-down"]` unsupported_poor_fit models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[4] & nonsupport$support_levels == "unsupported_poor_fit" & nonsupport$direction == "top-down"], "%", sep = "")`). 

* The top-down results for `r unique(nonsupport$rel_species)[3]` include `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[3] & nonsupport$support_levels == "unsupported_wrong_direction" & nonsupport$direction == "top-down"]` unsupported_wrong_direction models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[3] & nonsupport$support_levels == "unsupported_wrong_direction" & nonsupport$direction == "top-down"], "%", sep = "")`), `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[3] & nonsupport$support_levels == "unsupported_unclear_siv" & nonsupport$direction == "top-down"]` unsupported_unclear_siv models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[3] & nonsupport$support_levels == "unsupported_unclear_siv" & nonsupport$direction == "top-down"], "%", sep = "")`), and `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[3] & nonsupport$support_levels == "unsupported_poor_fit" & nonsupport$direction == "top-down"]` unsupported_poor_fit models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[3] & nonsupport$support_levels == "unsupported_poor_fit" & nonsupport$direction == "top-down"], "%", sep = "")`). 

* The top-down results for `r unique(nonsupport$rel_species)[2]` include `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[2] & nonsupport$support_levels == "unsupported_wrong_direction" & nonsupport$direction == "top-down"]` unsupported_wrong_direction models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[2] & nonsupport$support_levels == "unsupported_wrong_direction" & nonsupport$direction == "top-down"], "%", sep = "")`), `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[2] & nonsupport$support_levels == "unsupported_unclear_siv" & nonsupport$direction == "top-down"]` unsupported_unclear_siv models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[2] & nonsupport$support_levels == "unsupported_unclear_siv" & nonsupport$direction == "top-down"], "%", sep = "")`), and `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[2] & nonsupport$support_levels == "unsupported_poor_fit" & nonsupport$direction == "top-down"]` unsupported_poor_fit models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[2] & nonsupport$support_levels == "unsupported_poor_fit" & nonsupport$direction == "top-down"], "%", sep = "")`). 

* The results for `r unique(nonsupport$rel_species)[1]` include `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[1] & nonsupport$support_levels == "unsupported_wrong_direction" & nonsupport$direction == "top-down"]` unsupported_wrong_direction models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[1] & nonsupport$support_levels == "unsupported_wrong_direction" & nonsupport$direction == "top-down"], "%", sep = "")`), `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[1] & nonsupport$support_levels == "unsupported_unclear_siv" & nonsupport$direction == "top-down"]` unsupported_unclear_siv models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[1] & nonsupport$support_levels == "unsupported_unclear_siv" & nonsupport$direction == "top-down"], "%", sep = "")`), and `r nonsupport$num_lvl[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[1] & nonsupport$support_levels == "unsupported_poor_fit" & nonsupport$direction == "top-down"]` unsupported_poor_fit models (`r paste(nonsupport$percent[nonsupport$PPC_setting == "normal" & nonsupport$rel_species == unique(nonsupport$rel_species)[1] & nonsupport$support_levels == "unsupported_poor_fit" & nonsupport$direction == "top-down"], "%", sep = "")`). 
...
files = files[!grepl("supported_preferred_models", files)]
...
#
##
### Also import the newly split results per sp pair
...
s_file = files[grepl("species-pair", files)]
# import it 
preform_sp_cf = read.csv(paste(wd, s_file, sep = ""))

```

```{r Prep plotting data per model CF}
...
summary(ppc_values_cf$Interaction_Estimate) # there are a few big negative values! 
# long produced some even larger values!!
ppc_values_cf[ppc_values_cf$Interaction_Estimate < -2, c("counter_factual_test", "Species_Pair")] # only 4 here tho. 

# will be in the supplement anyway, so ok to leave in. 
# inspect further
anyNA(ppc_values_cf$Interaction_Estimate) # must be F! 

## thin results to only be preferred models 
ppc_values_cf = ppc_values_cf[ppc_values_cf$preference == "preferred", ]

## condense support levels for easier plotting
ppc_values_cf$support_simple[grepl("unsupported", ppc_values_cf$support)] = "unsupported"
...
## and remove two tests that are no longer needed for visualization
ppc_values_cf = ppc_values_cf[! ppc_values_cf$counter_factual_test %in% c("remove_community_detections"), ]
...
ppc_values_cf$counter_factual_test = factor(ppc_values_cf$counter_factual_test,
                                            levels = c("original_test", "site_matching_ThaiEFC",
                                                       #"isolate_abundance_declines",
                                                       # "remove_community_detections",
                                                       "isolate_extirpations",
                                                       "isolate_altitude", "isolate_FLII",
                                                       "isolate_HFP"))
```

```{r Prep plotting data per species pair CF}
#
##
### Repeat the process, but this time for ppc_values_cf 

## first copy over preform to a new data frame 
plot_dat_sp_cf = preform_sp_cf
# and determine the relevant species
plot_dat_sp_cf$rel_species[grepl("Panthera_tigris", plot_dat_sp_cf$pair_id)] = "Panthera_tigris"
plot_dat_sp_cf$rel_species[grepl("Panthera_pardus", plot_dat_sp_cf$pair_id)] = "Panthera_pardus"
plot_dat_sp_cf$rel_species[grepl("Cuon_alpinus", plot_dat_sp_cf$pair_id)] = "Cuon_alpinus"
plot_dat_sp_cf$rel_species[grepl("Neofelis_genus", plot_dat_sp_cf$pair_id)] = "Neofelis_genus"

## set order manually b/c order is silly on graph
plot_dat_sp_cf$rel_species = factor(plot_dat_sp_cf$rel_species, levels = c("Neofelis_genus","Cuon_alpinus",
                                                               "Panthera_pardus","Panthera_tigris"))
table(plot_dat_sp_cf$rel_species)

## create a new column for simple groupings: top-down, bottom-up and unsupported
plot_dat_sp_cf$support_simple = plot_dat_sp_cf$direction
plot_dat_sp_cf$support_simple_conserv = plot_dat_sp_cf$direction

### edit support_simple to allow for unsupported mods
plot_dat_sp_cf$support_simple[grepl("unsupported", plot_dat_sp_cf$support)] = "unsupported"
table(plot_dat_sp_cf$support_simple)

### and do the same for conservative values
plot_dat_sp_cf$support_simple_conserv[grepl("unsupported", plot_dat_sp_cf$support_conserv)] = "unsupported"
table(plot_dat_sp_cf$support_simple_conserv) # same as above, this is redundant! 

## remove NA values from plot_dat_sp_cf if present --> from non-completed mods
if(any(is.na(plot_dat_sp_cf$Interaction_Estimate))){
  plot_dat_sp_cf = plot_dat_sp_cf[! is.na(plot_dat_sp_cf$Interaction_Estimate), ]
}

## Check if there are extreme species interaction values (> |2|)
summary(plot_dat_sp_cf$Interaction_Estimate)  # some large values overall

## Check pref models
summary(plot_dat_sp_cf$Interaction_Estimate[plot_dat_sp_cf$preference == "preferred"]) # narrow range here, all good! 

## thin results to only be the same species pairs as the models dataframe 
plot_dat_sp_cf = plot_dat_sp_cf[plot_dat_sp_cf$preference == "preferred", ]

## set factors so unsupported is in the back 
plot_dat_sp_cf$support_simple = factor(plot_dat_sp_cf$support_simple, levels = c("top-down", "bottom-up", "unsupported"))
plot_dat_sp_cf$support_simple_conserv = factor(plot_dat_sp_cf$support_simple_conserv, levels = c("top-down", "bottom-up", "unsupported"))


## make the support column more informative
plot_dat_sp_cf$support[plot_dat_sp_cf$support == "Supported" & plot_dat_sp_cf$direction == "top-down"] = "top-down"
#plot_dat_sp_cf$support_conserv[plot_dat_sp_cf$support_conserv == "Supported" & plot_dat_sp_cf$direction == "top-down"] = "top-down"
plot_dat_sp_cf$support[plot_dat_sp_cf$support == "Supported" & plot_dat_sp_cf$direction == "bottom-up"] = "bottom-up"
#plot_dat_sp_cf$support_conserv[plot_dat_sp_cf$support_conserv == "Supported" & plot_dat_sp_cf$direction == "bottom-up"] = "bottom-up"
## Just to make code work, copy supports
plot_dat_sp_cf$support_conserv = plot_dat_sp_cf$support

## make letters all lowercase
plot_dat_sp_cf$support= tolower(plot_dat_sp_cf$support)
plot_dat_sp_cf$support_conserv = tolower(plot_dat_sp_cf$support_conserv)

## and set factor level 
plot_dat_sp_cf$support = factor(plot_dat_sp_cf$support, levels = c("top-down", "bottom-up", "unsupported"))
plot_dat_sp_cf$support_conserv = factor(plot_dat_sp_cf$support_conserv, levels = c("top-down", "bottom-up", "unsupported"))

### Verify plot_dat_sp_cf has the correct info 
table(plot_dat_sp_cf$rel_species[plot_dat_sp_cf$direction == "top-down"])
table(plot_dat_sp_cf$rel_species[plot_dat_sp_cf$direction == "bottom-up"]) # all critters accounted for 
## double check support
table(plot_dat_sp_cf$support[plot_dat_sp_cf$direction == "top-down"])
table(plot_dat_sp_cf$support_conserv[plot_dat_sp_cf$direction == "top-down"])
table(plot_dat_sp_cf$support[plot_dat_sp_cf$direction == "bottom-up"])
table(plot_dat_sp_cf$support_conserv[plot_dat_sp_cf$direction == "bottom-up"]) # All good!

## Set the facet wrap factor levels for nice ordering
plot_dat_sp_cf$counter_factual_test = factor(plot_dat_sp_cf$counter_factual_test,
                                             levels = c("original_test", "site_matching_ThaiEFC",
                                                        #"isolate_abundance_declines",
                                                        # "remove_community_detections",
                                                        "isolate_extirpations",
...
```{r Decide to use models or sp pairs}
## Make this a conditional statement so we dont have to change the code below! 
if(plot_type == "species-pairs"){
  plot_dat_cf = plot_dat_sp_cf
  hist_y = "Number of species pairs"
} 

if(plot_type == "models"){
  plot_dat_cf = ppc_values_cf
  hist_y = "Number of pairwise co-abundance models"
}
```
...
if(plot_type == "species-pairs"){
  sum_table_cf = ddply(plot_dat_cf, .(counter_factual_test, support_simple), summarize,
                  num_lvl = length(pair_id))
}
if(plot_type == "models"){
  sum_table_cf = ddply(plot_dat_cf, .(counter_factual_test, support_simple), summarize,
...
sum_table_cf$percent[sum_table_cf$counter_factual_test == "original_test"] = round(sum_table_cf$num_lvl[sum_table_cf$counter_factual_test == "original_test"] /  length(plot_dat_cf$counter_factual_test[plot_dat_cf$counter_factual_test == "original_test"]) * 100, 1)

sum_table_cf$percent[sum_table_cf$counter_factual_test == "site_matching_ThaiEFC"] = round(sum_table_cf$num_lvl[sum_table_cf$counter_factual_test == "site_matching_ThaiEFC"] / length(plot_dat_cf$counter_factual_test[plot_dat_cf$counter_factual_test == "site_matching_ThaiEFC"]) * 100, 1)

# sum_table_cf$percent[sum_table_cf$counter_factual_test == "isolate_abundance_declines"] = round(sum_table_cf$num_lvl[sum_table_cf$counter_factual_test == "isolate_abundance_declines"] / length(plot_dat_cf$counter_factual_test[plot_dat_cf$counter_factual_test == "isolate_abundance_declines"]) * 100, 1)

# sum_table_cf$percent[sum_table_cf$counter_factual_test == "remove_community_detections"] = round(sum_table_cf$num_lvl[sum_table_cf$counter_factual_test == "remove_community_detections"] / length(plot_dat_cf$counter_factual_test[plot_dat_cf$counter_factual_test == "remove_community_detections"]) * 100, 1)

sum_table_cf$percent[sum_table_cf$counter_factual_test == "isolate_extirpations"] = round(sum_table_cf$num_lvl[sum_table_cf$counter_factual_test == "isolate_extirpations"] / length(plot_dat_cf$counter_factual_test[plot_dat_cf$counter_factual_test == "isolate_extirpations"]) * 100, 1)

sum_table_cf$percent[sum_table_cf$counter_factual_test == "isolate_altitude"] = round(sum_table_cf$num_lvl[sum_table_cf$counter_factual_test == "isolate_altitude"] / length(plot_dat_cf$counter_factual_test[plot_dat_cf$counter_factual_test == "isolate_altitude"]) * 100, 1)

sum_table_cf$percent[sum_table_cf$counter_factual_test == "isolate_HFP"] = round(sum_table_cf$num_lvl[sum_table_cf$counter_factual_test == "isolate_HFP"] /      length(plot_dat_cf$counter_factual_test[plot_dat_cf$counter_factual_test == "isolate_HFP"]) * 100, 1)

sum_table_cf$percent[sum_table_cf$counter_factual_test == "isolate_FLII"] = round(sum_table_cf$num_lvl[sum_table_cf$counter_factual_test == "isolate_FLII"] /     length(plot_dat_cf$counter_factual_test[plot_dat_cf$counter_factual_test == "isolate_FLII"]) * 100, 1)
...
counterfactual_siv_hist = 
  ggplot(plot_dat_cf, aes(x = Interaction_Estimate, fill = support_simple))+
  geom_histogram(aes(y=after_stat(count)), colour="black", binwidth = .1, alpha = 1)+
  theme_classic()+
  geom_vline(aes(xintercept = 0), linetype = "dashed", color = "firebrick4", alpha = .5)+
  scale_fill_manual(values = new_colors) +
  scale_y_continuous(breaks = seq(0, max(table(plot_dat_cf$support_simple)), by = 2)) + 
  labs(x = "Species Interaction Value", y = hist_y, color = NULL)+
...
The plots and data used to inform this document are saved in the GitHub Repository here: `figures/step4_output_figs/` & `figures/step4_output_summarized_results_for_figs/`.
...
for(i in 1:length(all_plots)){
  ...
  if(grepl("concept", n)){
    ggsave(path, p, width = 13, height = 4, units = "in", dpi = 600)
  }
  if(grepl("unsup", n)){
    ggsave(path, p, width = 10, height = 10, units = "in", dpi = 600)
  }
  ...
  if(grepl("community", n) & grepl("all_large_carn", n)){
    ggsave(path, p, width = 12, height = 4, units = "in", dpi = 600)
  }
  if(grepl("preferred", n) & grepl("all_large_carn", n)){
    ggsave(path, p, width = 12, height = 4, units = "in", dpi = 600)
  }
  if(grepl("counterfactual", n)){
    ggsave(path, p, width = 15, height = 7, units = "in", dpi = 600)
  }
...
## and counterfactuals
#make a path
path = paste(wd, "percentages_for_counter-factual_histograms_", plot_type, "_", date, ".csv", sep = "")
# and save! 
