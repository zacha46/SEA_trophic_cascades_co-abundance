...
if(setting == "SIMULATION"){
  ni <- 5000;  nt <- 10; nb <- 1000; nc <- 4; na = NULL 
}
...
index <- readRDS("simulations_vJMS/simulation_parameter_grid.rds")[slurm,'index']

# Read in correct data frame from master list
sim <- readRDS("simulations_vJMS/sim_data_list.rds")[[index]]
...
# What to keep from the model?

# The extract table output, with some more params..
# the p-values and c-hat values
# No need to save input data because we can use the original sim file for plotting.
# just need to create plotting utilities which plot based on scenario - fun!
